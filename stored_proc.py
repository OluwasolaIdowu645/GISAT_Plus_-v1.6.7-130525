...
DROP TABLE IF EXISTS `validation_linelist`.ahd_linelist_for_ex;
CREATE TABLE `validation_linelist`.ahd_linelist_for_ex AS
...
CONCAT(p2.identifier, "-", 
@DATIMCode) Pepid_datim,
p2.identifier Pepid, 
a.person_id, 
k.gender Sex, 
i.encounter_datetime Date_Enrolled, 
h.ARTStartDate,
b.Indication_for_CD4, 
IF(g.CD4_LFA_Result="LessThan200", "<200", ">200") CD4_LFA_Result,
dt.Date_of_CD4_LFA_Result, 
cp.CD4_Percent,
    CASE 
        WHEN cp.CD4_Percent IS NULL THEN NULL 
        ELSE cpd.CD4_Percent_Date 
    END AS CD4_Percent_Date,
cc.CD4_Count,
ccd.CD4_Count_Date,
d.TB_LF_LAM_Result, 
d.Date_TB_LF_LAM_Result_Received, 
e.Serology_For_CrAg_Result, 
e.Date_Serology_For_CrAg_Result_Received, 
f.CSF_For_CrAg_Result, 
f.Date_CSF_For_CrAg_Result_Received,
j.ICE_WHO_Staging,
cr.Cryptococcal_Screening_Status,
 CONCAT(LEFT(UUID(), 4),RIGHT(YEAR(@Reporting_Date), 2),"-" , LEFT(UUID(), 5),
LPAD(MONTH(@Reporting_Date), 2, '0'),"-", LEFT(UUID(), 3), LPAD(DAY(@Reporting_Date), 2, '0')) List_Parameters
...
LEFT JOIN (SELECT person_id, ConceptName(`value_coded`) CD4_LFA_Result, encounter_id 
FROM obs WHERE concept_id = 167088 AND voided=0) g ON a.person_id = g.person_id AND a.encounter_id = g.encounter_id
LEFT JOIN (SELECT person_id, encounter_id, obs_datetime AS Date_of_CD4_LFA_Result, ConceptName(value_coded) AS CD4_LFA_Result FROM obs
WHERE concept_id = 164989 AND voided = 0
) dt ON a.person_id = dt.person_id AND a.encounter_id = dt.encounter_id

-- check --
LEFT JOIN (SELECT person_id, ConceptName(`value_coded`) CD4_Percent, encounter_id 
FROM obs WHERE concept_id = 730 AND voided=0) cp ON a.person_id = cp.person_id AND a.encounter_id = cp.encounter_id
LEFT JOIN (SELECT person_id, encounter_id, obs_datetime AS CD4_Percent_Date, ConceptName(value_coded) AS CD4_Percent FROM obs
WHERE concept_id = 164989 AND voided = 0
) cpd ON cp.person_id = cpd.person_id AND cp.encounter_id = cpd.encounter_id

LEFT JOIN (SELECT person_id, ConceptName(`value_coded`) CD4_Count, encounter_id 
FROM obs WHERE concept_id = 5497 AND voided=0) cc ON a.person_id = cc.person_id AND a.encounter_id = cc.encounter_id
LEFT JOIN (SELECT person_id, encounter_id, obs_datetime AS CD4_Count_Date, ConceptName(value_coded) AS CD4_Count FROM obs
WHERE concept_id = 164989 AND voided = 0
) ccd ON cc.person_id = ccd.person_id AND ccd.encounter_id = cpd.encounter_id
-- end-check --

LEFT JOIN (
  SELECT person_id, MAX(obs_datetime) AS obs_datetime,
    CASE value_coded
      WHEN 167131 THEN 'Not Screened'
      WHEN 167132 THEN 'Screened for Cryptococcal'
      WHEN 167133 THEN 'Cr Ag Negative'
      WHEN 167134 THEN 'Cr Ag Positive'
      WHEN 167135 THEN 'Cr Ag Positive Commenced on Pre-Emptive Therapy'
      WHEN 167136 THEN 'Diagnosed With Cryptococcal Meningitis'
      WHEN 167137 THEN 'Commenced Treatment For Cryptococcal Meningitis'
      WHEN 167138 THEN 'Ccompleted Treatment For Cryptococcal Meningitis'
    END AS Cryptococcal_Screening_Status
  FROM obs
  WHERE concept_id = 167130
    AND value_coded IN (167131, 167132, 167133, 167134, 167135, 167136, 167137, 167138)
    AND voided = 0
    AND encounter_id IN (
      SELECT encounter_id FROM encounter WHERE form_id = 14
    )
  GROUP BY person_id
) cr ON a.person_id = cr.person_id
...
SELECT * FROM `validation_linelist`.`ahd_linelist_for_ex`;
...
DROP TABLE IF EXISTS `validation_linelist`.Full_Linelist_for_Quick_List_Update;
CREATE TABLE `validation_linelist`.Full_Linelist_for_Quick_List_Update AS
...
DROP TABLE IF EXISTS `validation_linelist`.Last_Encounter_ID;
CREATE TABLE `validation_linelist`.Last_Encounter_ID AS
SELECT MAX(`encounter_id`) AS Last_Enc_id, MAX(`date_created`) AS Last_Date_time FROM `openmrs`.`encounter` WHERE `date_created` <= @Reporting_Date;

DROP TABLE IF EXISTS `validation_linelist`.last_run_date;
CREATE TABLE `validation_linelist`.last_run_date AS
SELECT DATE(@Reporting_Date) Last_Run_Date;

SELECT * FROM validation_linelist.Full_Linelist_for_Quick_List_Update;
    END
""")
...
DROP TABLE IF EXISTS `validation_linelist`.otz_linelist_for_ex;
CREATE TABLE `validation_linelist`.otz_linelist_for_ex AS
...
TIMESTAMPDIFF(YEAR, per.birthdate, CURDATE()) AS Current_Age, CASE WHEN TIMESTAMPDIFF(YEAR, per.birthdate, CURDATE()) >= 25 THEN 'Eligible for transitioning into Adulthood'
    ELSE NULL
END AS Transition_Eligibility,
e.`patient_id`, p2.`identifier` Pepid,  e.`encounter_datetime` Date_Enrolled_Into_OTZ, 
d.OTZ_Disclosure_Status, f.OTZ_Date_of_Full_Disclosure, b.Enrolled_Into_OTZ_Plus, c.Date_Enrolled_Into_OTZ_Plus, dc.Date_of_Commencement_of_OTZ_Module, g.Positive_Living, h.Positive_Living_Comp_Date, Treatment_Literacy,
j.Treatment_Literacy_Comp_Date, k.Adolescents_Participation, l.Adolescents_Participation_Comp_Date,m.Leadership_Training,n.Leadership_Training_Comp_Date,o.Peer_to_Peer_Mentorship,
p.Peer_to_Peer_Mentorship_Comp_Date,q.Role_of_OTZ_in_95_95_95,r.Role_of_OTZ_in_95_95_95_Comp_Date, ot.OTZ_Champion_Orientation, otz.OTZ_Champion_Orientation_Comp_Date, nom.Number_of_OTZ_Modules_Completed,
dcc.Date_of_Final_Module_Completion, s.Transitioned_to_Adult_Clinic,t.Date_Transitioned_to_Adult_Clinic,u.OTZ_Program_Outcome, CONCAT(LEFT(UUID(), 4),RIGHT(YEAR(@Reporting_Date), 2),"-" , LEFT(UUID(), 5),
LPAD(MONTH(@Reporting_Date), 2, '0'),"-", LEFT(UUID(), 3), LPAD(DAY(@Reporting_Date), 2, '0')) List_Parameters
FROM `encounter` e 
LEFT JOIN (SELECT o.`person_id`, COUNT(*) AS Number_of_OTZ_Modules_Completed FROM obs o
WHERE o.`concept_id` IN (166261, 166262, 166263, 166264, 166265, 166266, 166268) AND o.`voided` = 0 AND o.`value_datetime` IS NOT NULL GROUP BY o.`person_id`) nom ON nom.person_id = e.`patient_id`
...
LEFT JOIN (SELECT o.`person_id`, o.`encounter_id`, ConceptName(o.`concept_id`), ConceptName(o.`value_coded`) 'OTZ_Disclosure_Status' FROM obs o 
WHERE o.`concept_id` = 166270 AND o.`voided`=0)d ON d.person_id=e.`patient_id` AND d.encounter_id=e.`encounter_id`
LEFT JOIN (SELECT o.`person_id`, o.`encounter_id`, ConceptName(o.`concept_id`), o.`value_datetime` 'OTZ_Date_of_Full_Disclosure' FROM obs o 
WHERE o.`concept_id` = 166271 AND o.`voided`=0)f ON f.person_id=e.`patient_id` AND f.encounter_id=e.`encounter_id`

LEFT JOIN (SELECT o.`person_id`, o.`encounter_id`, ConceptName(o.`concept_id`), ConceptName(o.`value_coded`) 'OTZ_Champion_Orientation' FROM obs o 
WHERE o.`concept_id` = 166267 AND o.`voided`=0)ot ON ot.person_id=e.`patient_id` AND ot.encounter_id=e.`encounter_id`
LEFT JOIN (SELECT o.`person_id`, o.`encounter_id`, ConceptName(o.`concept_id`), o.`value_datetime` 'OTZ_Champion_Orientation_Comp_Date' FROM obs o 
WHERE o.`concept_id` = 166268 AND o.`voided`=0)otz ON otz.person_id=e.`patient_id` AND otz.encounter_id=e.`encounter_id`
LEFT JOIN (SELECT o.`person_id`, o.`encounter_id`, ConceptName(o.`concept_id`), o.`value_datetime` 'Date_of_Final_Module_Completion' FROM obs o 
WHERE o.`concept_id` = 166268 AND o.`voided`=0)dcc ON dcc.person_id=e.`patient_id` AND dcc.encounter_id=e.`encounter_id`
...
LEFT JOIN (
    SELECT 
        o.person_id,
        MIN(o.value_datetime) AS Date_of_Commencement_of_OTZ_Module
    FROM obs o
    WHERE o.concept_id IN (166261, 166262, 166263, 166264, 166265, 166266, 166268)
      AND o.voided = 0
    GROUP BY o.person_id
) dc ON dc.person_id = e.patient_id
...
LEFT JOIN person per ON e.`patient_id` = per.`person_id`
WHERE e.`encounter_type`=36 AND e.`form_id`=73 AND e.`voided`=0 GROUP BY e.`patient_id`, e.`encounter_id`;

SELECT * FROM `validation_linelist`.`otz_linelist_for_ex`;
  END
""")
...
DROP TABLE IF EXISTS `validation_linelist`.`full_pharmacy_complement_for_ex`;
CREATE TABLE `validation_linelist`.`full_pharmacy_complement_for_ex` AS
...
SELECT 
    ...
    @FacilityName FacilityName,
    @DATIMCode DATIMCode,
    CONCAT(p1.`identifier`, "-", @DATIMCode) Pepid_datim,
    p1.`identifier` PEPID,
    a.patient_id,
    a.LastPickupDate,
    a.`encounter_id`,
    p.RegLineAtStart, -- new field
    q.ARTRegAtStart, -- new field
    b.DaysOfARVRefill,
    CASE
        WHEN d.Next_Pickup_Date <= DATE_ADD(@Reporting_Date, INTERVAL 28 DAY) THEN 'Active'
        WHEN p2.death_date IS NOT NULL THEN 'Died'
        WHEN p2.voided = 1 THEN 'Transferred Out'
        ELSE 'Inactive'
    END AS CurrentARTStatus_28Days, -- new field
    c.Pill_Balance,
    d.Next_Pickup_Date,
    e.CurrentARTReg,
    n.CurrentRegLine,
    ...
    m.DDD_Fac_Disp,
    CASE 
        WHEN s.value_coded = 165773 THEN 'New'
        WHEN s.value_coded = 165662 THEN 'Refill'
        WHEN s.value_coded = 165772 THEN 'Switch'
        WHEN s.value_coded = 165665 THEN 'Substitution'
        ELSE ''
    END AS `Switch/Substitution`, -- new field
    o.PregStatus,
    CONCAT(LEFT(UUID(), 4),RIGHT(YEAR(@Reporting_Date), 2),"-" , LEFT(UUID(), 5),
LPAD(MONTH(@Reporting_Date), 2, '0'),"-", LEFT(UUID(), 3), LPAD(DAY(@Reporting_Date), 2, '0')) List_Parameters
...
FROM (
    SELECT 
        @row_no := IF(@prev_val = e.patient_id, @row_no + 1, 1) AS Occ,
        @prev_val := e.patient_id AS patient_id,
        e.`encounter_datetime` LastPickupDate,
        e.`encounter_id`
    FROM `encounter` e, 
         (SELECT @row_no := 0) X, 
         (SELECT @prev_val := '') Y
    WHERE e.`encounter_type` = 13 
      AND e.`form_id` = 27 
      AND e.`voided` = 0 
      AND e.`encounter_datetime` <= @Reporting_Date 
    ORDER BY e.patient_id, e.`encounter_datetime` DESC
) a
...
LEFT JOIN (
    SELECT 
        ...
        o.encounter_id, 
        o.`obs_id`, 
        o.`value_numeric` DaysOfARVRefill 
    FROM `obs` o 
    WHERE o.concept_id = 159368 
      AND o.`voided` = 0
      AND obs_group_id IN (
          SELECT obs_id FROM obs 
          WHERE concept_id = 162240 AND `voided` = 0
      )
...
LEFT JOIN (
    SELECT 
        person_id, 
        CAST(`value_text` AS UNSIGNED) Pill_Balance, 
        encounter_id 
    FROM obs 
    WHERE concept_id = 166406 
      AND `voided` = 0
...
LEFT JOIN (
    SELECT 
        person_id, 
        `value_datetime` Next_Pickup_Date, 
        encounter_id 
    FROM obs 
    WHERE concept_id = 5096 
      AND `voided` = 0
) d ON d.encounter_id = a.encounter_id
...
LEFT JOIN (
    SELECT 
        o.encounter_id, 
        o.`value_coded`, 
        ConceptName(o.`value_coded`) CurrentARTReg 
    FROM `obs` o 
    WHERE `concept_id` IN (164506, 164507, 164513, 164514, 165702, 165703) 
      AND o.`voided` = 0 
      AND obs_group_id IS NULL
) e ON e.encounter_id = a.encounter_id

-- Last Current Regimen Line
LEFT JOIN (
    SELECT 
        o.`person_id`, 
        o.`encounter_id`, 
        o.`obs_datetime`, 
        ConceptName(o.`value_coded`) CurrentRegLine  
    FROM `obs` o
    WHERE o.`concept_id` = 165708 
      AND o.`voided` = 0
) f ON f.encounter_id = a.encounter_id
...
LEFT JOIN (
    SELECT 
        ...
        o.`encounter_id`, 
        o.`obs_id`, 
        GROUP_CONCAT(DISTINCT ConceptName(o.`value_coded`) SEPARATOR '-') ARV_Drug_Strength 
    FROM `obs` o
    WHERE o.concept_id = 165725 
      AND o.`voided` = 0 
      AND obs_group_id IN (
          SELECT obs_id FROM obs 
    ...
    GROUP BY o.`encounter_id`
) g ON g.encounter_id = a.encounter_id
...
LEFT JOIN (
    SELECT 
        ...
        o.encounter_id, 
        o.`obs_id`, 
        ConceptName(o.`value_coded`) OI_Drug_CTX 
    FROM `obs` o 
    WHERE o.concept_id = 165727 
      AND o.`voided` = 0
      AND obs_group_id IN (
          SELECT obs_id FROM obs 
          WHERE concept_id = 165726 
            AND `voided` = 0
      ) 
      AND ConceptName(o.`value_coded`) LIKE 'CTX%'
) h ON h.encounter_id = a.encounter_id
...
LEFT JOIN (
    SELECT 
        ...
        o.encounter_id, 
        o.`obs_id`, 
        ConceptName(o.`value_coded`) OI_Drug_INH 
    FROM `obs` o 
    WHERE o.concept_id = 165727 
      AND o.`voided` = 0
      AND obs_group_id IN (
          ...
          WHERE concept_id = 165726 
            AND `voided` = 0 
            AND ConceptName(o.`value_coded`) LIKE 'IS%'
      )
) i ON i.encounter_id = a.encounter_id
...
LEFT JOIN (
    SELECT 
        ...
        o.encounter_id, 
        o.`obs_id`, 
        ConceptName(o.`value_coded`) CTX_Strength 
    FROM `obs` o 
    WHERE o.concept_id = 165725 
      AND o.`voided` = 0
      AND obs_group_id IN (
          SELECT obs_group_id FROM obs 
          WHERE value_coded = 165257 
            AND `voided` = 0
      )
) j ON j.encounter_id = a.encounter_id
...
LEFT JOIN (
    SELECT 
        ...
        o.encounter_id, 
        o.`obs_id`, 
        ConceptName(o.`value_coded`) INH_Strength 
    FROM `obs` o 
    WHERE o.concept_id = 165725 
      AND o.`voided` = 0
      AND obs_group_id IN (
          SELECT obs_group_id FROM obs 
          WHERE value_coded = 1679 
            AND `voided` = 0
      )
) k ON k.encounter_id = a.encounter_id
...
LEFT JOIN (
    SELECT 
        ...
        o.`encounter_id`, 
        o.`obs_group_id`,  
        ConceptName(o.`value_coded`) DSD_Model
    FROM `obs` o 
    WHERE o.`concept_id` = 166148 
      AND o.`voided` = 0
) l ON l.encounter_id = a.encounter_id

-- Facility Dispensing (DDD)
LEFT JOIN (
    SELECT 
        o.`person_id`, 
        o.`encounter_id`, 
        o.`obs_group_id`,  
        ConceptName(o.`value_coded`) DDD_Fac_Disp
    FROM `obs` o 
    WHERE o.`concept_id` IN (166276, 166363) 
      AND o.`voided` = 0
) m ON m.encounter_id = a.encounter_id

-- Current Regimen Line (Grouped)
LEFT JOIN (
    SELECT 
        o.`person_id`, 
        o.`encounter_id`, 
    ...
    WHERE o.`concept_id` = 165708 
      AND o.`voided` = 0 
    GROUP BY o.encounter_id
) n ON n.encounter_id = a.encounter_id
...
LEFT JOIN (
    SELECT 
        ...
        p.`gender`, 
        o.encounter_id, 
        ConceptName(o.`value_coded`) PregStatus 
    FROM `obs` o 
    LEFT JOIN `person` p 
        ON p.`person_id` = o.`person_id` 
       AND p.`voided` = 0
    WHERE o.`concept_id` = 165050 
      AND o.`voided` = 0 
      AND p.gender = 'F'
) o ON o.encounter_id = a.encounter_id


-- RegLineAtStart
LEFT JOIN (
    SELECT o.person_id, o.encounter_id, ConceptName(o.value_coded) AS RegLineAtStart
    FROM obs o
    INNER JOIN (
        SELECT person_id, MIN(obs_datetime) AS min_date
        FROM obs
        WHERE concept_id = 165708 AND voided = 0
        GROUP BY person_id
    ) first_obs ON o.person_id = first_obs.person_id AND o.obs_datetime = first_obs.min_date
    WHERE o.concept_id = 165708 AND o.voided = 0
) p ON p.encounter_id = a.encounter_id


-- ARTRegAtStart
LEFT JOIN (
    SELECT o.person_id, o.encounter_id, ConceptName(o.value_coded) AS ARTRegAtStart
    FROM obs o
    INNER JOIN (
        SELECT person_id, MIN(obs_datetime) AS min_date
        FROM obs
    ...
    WHERE o.concept_id IN (164506, 164507, 164513, 164514, 165702, 165703)
      AND o.voided = 0 AND o.obs_group_id IS NULL
) q ON q.encounter_id = a.encounter_id


-- Patient Identifier (PEPID)
LEFT JOIN `patient_identifier` p1 
    ON a.`patient_id` = p1.`patient_id` 
   AND p1.`identifier_type` = 4
   
-- CurrentARTStatus_28Days
-- Person (for death_date and voided)
LEFT JOIN person p2 ON p2.person_id = a.patient_id

-- Switch and Substitution
-- LEFT JOIN obs s 
--     ON s.person_id = a.patient_id
--     AND s.concept_id = 162240
--     AND s.voided = 0
LEFT JOIN (
    SELECT s1.*
    FROM obs s1
    INNER JOIN (
        ...
        FROM obs 
        WHERE concept_id = 165774 AND voided = 0
        GROUP BY person_id
    ) s2 ON s1.person_id = s2.person_id AND s1.obs_datetime = s2.max_date
    WHERE s1.concept_id = 165774 AND s1.voided = 0
) s ON s.encounter_id = a.encounter_id

WHERE a.Occ = 1;

SELECT * FROM `validation_linelist`.`full_pharmacy_complement_for_ex`;
  END
""")
...
DROP TABLE IF EXISTS `validation_linelist`.ctd_linelist_for_ex;
CREATE TABLE `validation_linelist`.ctd_linelist_for_ex AS
...
p2.identifier Pepid, 
    ...
    (SELECT o.value_datetime FROM obs o WHERE o.encounter_id = e.encounter_id AND o.concept_id = 165463 AND o.voided = 0 LIMIT 1) AS Tracking_Date,
    (SELECT ConceptName(o.value_coded) FROM obs o WHERE o.encounter_id = e.encounter_id AND o.concept_id = 165460 AND o.voided = 0 LIMIT 1) AS Reason_For_Tracking,
    (SELECT o.value_text FROM obs o WHERE o.encounter_id = e.encounter_id AND o.concept_id = 161135 AND o.voided = 0 LIMIT 1) AS Partner_Full_Name,
    (SELECT o.value_text FROM obs o WHERE o.encounter_id = e.encounter_id AND o.concept_id = 160641 AND o.voided = 0 LIMIT 1) AS Address_of_Tx_Supporter,
    (SELECT o.value_text FROM obs o WHERE o.encounter_id = e.encounter_id AND o.concept_id = 159635 AND o.voided = 0 LIMIT 1) AS Contact_Phone_No,
    (SELECT o.value_datetime FROM obs o WHERE o.encounter_id = e.encounter_id AND o.concept_id = 165461 AND o.voided = 0 LIMIT 1) AS Date_of_Last_Actual,
    (SELECT o.value_datetime FROM obs o WHERE o.encounter_id = e.encounter_id AND o.concept_id = 165778 AND o.voided = 0 LIMIT 1) AS Date_Missed_Scheduled_App,
    (SELECT o.value_datetime FROM obs o WHERE o.encounter_id = e.encounter_id AND o.concept_id = 165463 AND o.voided = 0 LIMIT 1) AS ContAttempt_Date1,
    (SELECT ConceptName(o.value_coded) FROM obs o WHERE o.encounter_id = e.encounter_id AND o.concept_id = 165464 AND o.voided = 0 LIMIT 1) AS Who_Attempted_Contact1,
    (SELECT ConceptName(o.value_coded) FROM obs o WHERE o.encounter_id = e.encounter_id AND o.concept_id = 165465 AND o.voided = 0 LIMIT 1) AS Mode_of_Communication1,
    (SELECT ConceptName(o.value_coded) FROM obs o WHERE o.encounter_id = e.encounter_id AND o.concept_id = 165466 AND o.voided = 0 LIMIT 1) AS Person_Contacted1,
    (SELECT ConceptName(o.value_coded) FROM obs o WHERE o.encounter_id = e.encounter_id AND o.concept_id = 165467 AND o.voided = 0 LIMIT 1) AS Reason_for_Defaulting1,    
    (SELECT ConceptName(o.value_coded) FROM obs o WHERE o.encounter_id = e.encounter_id AND o.concept_id = 165470 AND o.voided = 0 LIMIT 1) AS Reason_for_Discontinuation,
    (SELECT o.value_datetime FROM obs o WHERE o.encounter_id = e.encounter_id AND o.concept_id = 165469 AND o.voided = 0 LIMIT 1) AS Date_of_Discontinuation,
    (SELECT ConceptName(o.value_coded) FROM obs o WHERE o.encounter_id = e.encounter_id AND o.concept_id = 165586 AND o.voided = 0 LIMIT 1) AS Previous_ARV_exposure,
    (SELECT ConceptName(o.value_coded) FROM obs o WHERE o.encounter_id = e.encounter_id AND o.concept_id = 165776 AND o.voided = 0 LIMIT 1) AS Referred_for,
    (SELECT o.value_datetime 
 FROM obs o 
 WHERE o.encounter_id = e.encounter_id 
   AND o.concept_id = 165775 
   AND o.voided = 0 
 LIMIT 1) AS Date_Returned,

CASE 
    WHEN (SELECT o.value_datetime 
          FROM obs o 
          WHERE o.encounter_id = e.encounter_id 
            AND o.concept_id = 165775 
            AND o.voided = 0 
          LIMIT 1) IS NOT NULL 
    THEN 'Returned to Treatment'
    ELSE NULL
END AS Returned_to_Treatment_Status,
    (SELECT o.value_datetime FROM obs o WHERE o.encounter_id = e.encounter_id AND o.concept_id = 166152 AND o.voided = 0 LIMIT 1) AS Date_LTFU,
    (SELECT ConceptName(o.value_coded) FROM obs o WHERE o.encounter_id = e.encounter_id AND o.concept_id = 166157 AND o.voided = 0 LIMIT 1) AS Reason_LTFU,
    (SELECT ConceptName(o.value_coded) FROM obs o WHERE o.encounter_id = e.encounter_id AND o.concept_id = 5240 AND o.voided = 0 LIMIT 1) AS LTFU,
    (SELECT o.value_text FROM obs o WHERE o.encounter_id = e.encounter_id AND o.concept_id = 165459 AND o.voided = 0 LIMIT 1) AS Name_of_Tracker,
    (SELECT o.value_datetime FROM obs o WHERE o.encounter_id = e.encounter_id AND o.concept_id = 165777 AND o.voided = 0 LIMIT 1) AS Tracker_Sig_Date,
    (SELECT o.value_text FROM obs o WHERE o.encounter_id = e.encounter_id AND o.concept_id = 159495 AND o.voided = 0 LIMIT 1) AS Facility_Transferred_To,
    CONCAT(LEFT(UUID(), 4),RIGHT(YEAR(@Reporting_Date), 2),"-" , LEFT(UUID(), 5),
LPAD(MONTH(@Reporting_Date), 2, '0'),"-", LEFT(UUID(), 3), LPAD(DAY(@Reporting_Date), 2, '0')) List_Parameters

FROM encounter e
LEFT JOIN `patient_identifier` p2 ON e.`patient_id` = p2.`patient_id` AND p2.`identifier_type` = 4
...
WHERE 
    ...
    AND NOT EXISTS (
        ...
        AND e2.encounter_type = 15
        AND e2.form_id = 13
        AND e2.voided = 0
        AND e2.encounter_datetime <= @Reporting_Date
    );
SELECT * FROM `validation_linelist`.ctd_linelist_for_ex;
  END
""")
...
cur.execute("""DROP PROCEDURE IF EXISTS `Last_6_Viral_Load_Sample_Result`""")
cur.execute("""
  CREATE PROCEDURE `Last_6_Viral_Load_Sample_Result`()
  BEGIN
...
WHERE `property`= 'facility_datim_code'));
 ...
 SELECT 
        ...
        CONCAT(p2.identifier,'-',(SELECT `property_value` FROM `global_property` WHERE `property`= 'facility_datim_code')) AS Pepid_Datim,
        @row_no := IF(@prev_val = e.patient_id, @row_no + 1, 1) AS Occurrence, 
        @prev_val := e.patient_id AS id, 
        p2.`identifier` AS Pepid, e.`encounter_datetime` DateOfVL, b.VL, c.ViralLoadIndication, d.Sample_Collection,
        TIMESTAMPDIFF(YEAR, f.birthdate, @Reporting_Date) AS CurrentAge, b.Date_Result_Received
    FROM `encounter` e 
        LEFT JOIN `patient_identifier` p2 ON e.`patient_id` = p2.`patient_id` AND p2.`identifier_type` = 4 AND p2.`preferred` = 1
        LEFT JOIN obs ON e.`encounter_id` = obs.`encounter_id`
  /* -- ------------------------------------------------------------------------------------------------------------------------------------------*/
        LEFT JOIN (SELECT
        @row_no := IF(@prev_val = o.person_id, @row_no + 1, 1) AS Occurrence, 
        @prev_val := e.patient_id AS id,
        o.`obs_datetime` AS DateOfVL, 
        o.`value_numeric` AS VL, e.encounter_id, b.Date_Result_Received
    FROM `encounter` e 
	LEFT JOIN obs o ON o.encounter_id = e.encounter_id
        LEFT JOIN `patient_identifier` p2 ON o.`person_id` = p2.`patient_id` AND p2.`identifier_type` = 4 AND p2.`preferred` = 1
        LEFT JOIN (SELECT DISTINCT person_id, encounter_id, value_datetime AS Date_Result_Received FROM obs WHERE concept_id = 165987 AND voided=0)b ON b.encounter_id = e.encounter_id,
        (SELECT @row_no := 0) X, 
        (SELECT @prev_val := '') Y 
    WHERE 
        o.`concept_id` = 856 AND 
        e.`encounter_type` = 11 AND 
        e.`form_id` = 21 AND
        o.`voided` = 0
    ORDER BY 
        o.person_id, 
        o.`obs_datetime` DESC) b ON e.`encounter_id`= b.`encounter_id`
/*-- ------------------------------------------------------------------------------------------------------------------------------------------*/
     LEFT JOIN (SELECT e.patient_id, o.`obs_datetime`, ConceptName(o.`value_coded`) ViralLoadIndication , e.`encounter_id` FROM `obs` o
	LEFT JOIN `encounter` e ON o.`encounter_id`=e.`encounter_id`
	WHERE e.`encounter_type` = 11 AND e.`form_id` = 21 AND o.`concept_id` = 164980 AND o.`voided` = 0 AND  o.`person_id`= e.patient_id
	GROUP BY e.`encounter_id`) c ON e.encounter_id = c.encounter_id
/*-- ------------------------------------------------------------------------------------------------------------------------------------------*/
     LEFT JOIN (SELECT 
        @row_no := IF(@prev_val = o.person_id, @row_no + 1, 1) AS Occurrence, 
        @prev_val := e.patient_id AS id, 
        p2.`identifier` AS Pepid,
        o.`obs_datetime` AS Sample_Collection, e.encounter_id
    FROM `encounter` e 
	LEFT JOIN obs o ON o.encounter_id = e.encounter_id
        LEFT JOIN `patient_identifier` p2 ON o.`person_id` = p2.`patient_id` AND p2.`identifier_type` = 4 AND p2.`preferred` = 1,
        (SELECT @row_no := 0) X, 
        (SELECT @prev_val := '') Y 
    WHERE 
        o.`concept_id` = 159951 AND 
        e.`encounter_type` = 11 AND 
        e.`form_id` = 21 AND
        o.`voided` = 0
    ORDER BY 
        o.person_id, 
        o.`obs_datetime` DESC) d ON e.encounter_id = d.encounter_id
/*-- ------------------------------------------------------------------------------------------------------------------------------------------*/
    LEFT JOIN (SELECT `person_id`, `birthdate` FROM `person`) f ON e.`patient_id`=f.`person_id` ,
        (SELECT @row_no := 0) X, 
        (SELECT @prev_val := '') Y 
    WHERE 
        e.`encounter_type` = 11 AND obs.`concept_id` = 165765 AND value_coded = 2 AND 
        e.`form_id` = 21 AND 
        e.`voided` = 0
    ORDER BY 
        e.patient_id, 
    ...
    /*-- Create temporary tables for each of the 6 VL occurrences*/
    DROP TEMPORARY TABLE IF EXISTS b;
    CREATE TEMPORARY TABLE b AS SELECT * FROM a WHERE Occurrence = 1;

    DROP TEMPORARY TABLE IF EXISTS c;
    CREATE TEMPORARY TABLE c AS SELECT * FROM a WHERE Occurrence = 2;

    DROP TEMPORARY TABLE IF EXISTS d;
    CREATE TEMPORARY TABLE d AS SELECT * FROM a WHERE Occurrence = 3;

    DROP TEMPORARY TABLE IF EXISTS e;
    CREATE TEMPORARY TABLE e AS SELECT * FROM a WHERE Occurrence = 4;
    
    DROP TEMPORARY TABLE IF EXISTS f;
    CREATE TEMPORARY TABLE f AS SELECT * FROM a WHERE Occurrence = 5;
    
    DROP TEMPORARY TABLE IF EXISTS g;
    CREATE TEMPORARY TABLE g AS SELECT * FROM a WHERE Occurrence = 6;
   ...
   /* -- ------------*/
    ...
    @LGA LGA,
        ...
        a.Pepid, 
        a.CurrentAge,
        b.Sample_Collection AS 1st_Last_Sample_Collection,
        b.ViralLoadIndication AS 1st_Last_ViralLoadIndication, 
        b.DateOfVL AS 1st_Last_VLDate,
        b.VL AS 1st_Last_VL, b.Date_Result_Received 1st_Last_Date_Result_Received,
        c.Sample_Collection AS 2nd_Last_Sample_Collection,
        c.ViralLoadIndication AS 2nd_Last_ViralLoadIndication, 
        c.DateOfVL AS 2nd_Last_VLDate, 
        c.VL AS 2nd_Last_VL, c.Date_Result_Received 2nd_Last_Date_Result_Received,
        d.Sample_Collection AS 3rd_Last_Sample_Collection,
        d.ViralLoadIndication AS 3rd_Last_ViralLoadIndication, 
        d.DateOfVL AS 3rd_Last_VLDate, 
        d.VL AS 3rd_Last_VL, d.Date_Result_Received  3rd__Last_Date_Result_Received,
        e.Sample_Collection AS 4th_Last_Sample_Collection,
        e.ViralLoadIndication AS 4th_Last_ViralLoadIndication, 
        e.DateOfVL AS 4th_Last_VLDate, 
        e.VL AS 4th_Last_VL, e.Date_Result_Received  4th_Last_Date_Result_Received,
        f.Sample_Collection AS 5th_Last_Sample_Collection,
        f.ViralLoadIndication AS 5th_Last_ViralLoadIndication, 
        f.DateOfVL AS 5th_Last_VLDate, 
        f.VL AS 5th_Last_VL, f.Date_Result_Received  5th_Last_Date_Result_Received,
        g.Sample_Collection AS 6th_Last_Sample_Collection,
        g.ViralLoadIndication AS 6th_Last_ViralLoadIndication, 
        g.DateOfVL AS 6th_Last_VLDate, 
        g.VL AS 6th_Last_VL, g.Date_Result_Received  6th_Last_Date_Result_Received
    FROM 
        a 
        ...
        LEFT JOIN g ON g.id = a.id
        WHERE a.pepid IS NOT NULL
      GROUP BY a.id;
...
cur.execute("""DROP PROCEDURE IF EXISTS `Gap_Identification`""")
cur.execute("""
  CREATE PROCEDURE `Gap_Identification`()
  BEGIN
DROP TABLE IF EXISTS CIHP_listoffacility;
CREATE TABLE IF NOT EXISTS  CIHP_listoffacility (
  `FacilityID` DOUBLE DEFAULT NULL,
...
DROP TABLE IF EXISTS More_DrugDur_Cleaning;
CREATE TEMPORARY TABLE More_DrugDur_Cleaning AS
SELECT a.identifier, a.encounter_id, a.patient_id, a.encounter_datetime, a.Reg_Occur, b.Med_Dur_Occur FROM (
  SELECT
  p.`identifier`,
  e.`encounter_id`,
  e.`patient_id`,
  e.`encounter_datetime`, COUNT(o.`concept_id`) Reg_Occur,
  GROUP_CONCAT(DISTINCT c.`name` SEPARATOR ', ') AS 'All_Concept_Names'
FROM
  `encounter` e
  LEFT JOIN obs o
  ...
  LEFT JOIN `patient_identifier` p
    ON o.`person_id` = p.`patient_id`
WHERE e.form_id = 27
  AND p.`identifier_type` = 4
  AND e.`encounter_datetime` <= CURDATE()
  AND c.`locale` = 'en'
  AND o.`concept_id` = 165724
  AND o.`voided` = 0
GROUP BY e.`encounter_datetime`,
  ...
  e.`encounter_id`,
  e.`patient_id`,
  e.`encounter_datetime`, COUNT(o.`concept_id`) Med_Dur_Occur,
  GROUP_CONCAT(DISTINCT c.`name` SEPARATOR ', ') AS 'All_Concept_Names'
FROM
  `encounter` e
  LEFT JOIN obs o
...
WHERE e.form_id = 27
  ...
  AND o.`voided` = 0
GROUP BY e.`encounter_datetime`,
  o.`person_id`) b ON a.patient_id=b.patient_id AND a.encounter_datetime=b.encounter_datetime 
  WHERE b.Med_Dur_Occur < a.Reg_Occur ORDER BY encounter_datetime DESC;
/*DROP TABLE IF EXISTS ARTStarDate_Fillup;
CREATE TEMPORARY TABLE ARTStarDate_FillupAS
SELECT
  p.`identifier`,
  e.`encounter_id`,
  o.`person_id`,
  ...
  e.`form_id`,
  e.`encounter_datetime`,
  COUNT(DISTINCT o.`concept_id`) AS Concept_Completed,
  GROUP_CONCAT(
    DISTINCT o.`concept_id` SEPARATOR ', '
  ) 'All_Concept_Ids',
  GROUP_CONCAT(
    DISTINCT o.value_coded SEPARATOR ', '
  ) 'All_values',
  GROUP_CONCAT(DISTINCT c.`name` SEPARATOR ', ') AS 'All_Concept_Names'
FROM
  `encounter` e
  ...
  LEFT JOIN `patient_identifier` p
    ON o.`person_id` = p.`patient_id`
WHERE e.encounter_type = 14
  AND p.`identifier_type` = 4
  AND c.`locale` = 'en'
GROUP BY e.`encounter_datetime`,
  o.`person_id`;*/
DROP TABLE IF EXISTS ARVCommencement_Cleaning;
CREATE TEMPORARY TABLE ARVCommencement_Cleaning AS
SELECT
p.`identifier`,
  o.`obs_id`,
  o.`person_id`,
...
FROM
  ...
  LEFT JOIN `patient_identifier` p
    ON o.`person_id` = p.`patient_id`
  LEFT JOIN (SELECT o.`person_id`, o.`concept_id`, o.`obs_datetime`, o.`value_coded`, c2.`name` ARTRegAtStart  FROM `obs` o 
LEFT JOIN `concept_name` c2 ON o.`value_coded`=c2.`concept_id` AND c2.locale = 'en' AND c2.locale_preferred = 1
LEFT JOIN `encounter` e ON o.`encounter_id`=e.`encounter_id`
WHERE e.`encounter_type` = 25 AND e.`form_id` = 56 AND o.`concept_id` IN (164506, 164507, 164513, 164514, 165702, 165703) AND o.`voided` = 0 
GROUP BY o.`person_id`, o.`obs_datetime`) AS f
...
GROUP BY e.`encounter_datetime`,
  o.`person_id`;
DROP TABLE IF EXISTS HIVEnrollment_Cleaning;
CREATE TEMPORARY TABLE HIVEnrollment_Cleaning AS
SELECT
p.`identifier`,
  o.`obs_id`,
  o.`person_id`,
...
DROP TABLE IF EXISTS Address_Clean;
CREATE TEMPORARY TABLE Address_Clean AS
SELECT `person_id`,`preferred`,`address1`,`address2`,`city_village`,`state_province`,'LGA not Specified'Staus,`country` FROM `person_address` 
WHERE voided=0 AND (`city_village` IS NULL OR`city_village`='')
UNION ALL
SELECT `person_id`,`preferred`,`address1`,`address2`,`city_village`,`state_province`,'State not Specified'Staus,`country` FROM `person_address` 
WHERE voided=0 AND (`state_province`='' OR `state_province` IS NULL)
UNION ALL
SELECT `person_id`,`preferred`,`address1`,`address2`,`city_village`,`state_province`, 'LGA wrongly spelt'Staus,`country` FROM `person_address` 
WHERE voided=0 AND `city_village` LIKE '%_%' AND `city_village` NOT IN (SELECT `name` FROM `address_hierarchy_entry` WHERE `level_id`=3)
UNION ALL
SELECT `person_id`,`preferred`,`address1`,`address2`,`city_village`,`state_province`,'State wrongly spelt'Staus,`country` FROM `person_address` 
WHERE voided=0 AND `state_province` LIKE '%_%' AND `state_province` NOT IN (SELECT `name` FROM `address_hierarchy_entry` WHERE `level_id`=2)
UNION ALL
SELECT `person_id`,`preferred`,address1,CONCAT_WS(' ',`address1`,`address2`)address2,`city_village`,`state_province`,
IF(LENGTH(CONCAT_WS(' ',`address1`,`address2`))<16,'Address not descriptive enough',NULL)Staus,`country` FROM `person_address` 
WHERE voided=0;
DROP TABLE IF EXISTS Address_Clean2;
CREATE TEMPORARY TABLE Address_Clean2 AS
SELECT `person_id`,`preferred`,`address1`,`address2`,`city_village`,`state_province`,GROUP_CONCAT(DISTINCT `Staus` SEPARATOR ' | ') AS 'Status',`country` FROM Address_Clean GROUP BY person_id;
/****This very useful for better understanding*****
SELECT a.`person_id`,a.`preferred`,a.`address1`,a.`address2`,a.`city_village`,a.`state_province`,a.`country`, b.name state, 
b.`address_hierarchy_entry_id` stateId, c.`name` LGA, c.`parent_id` LGAId, d.Status, if(b.`address_hierarchy_entry_id`=c.`parent_id`,null, 'City/Village (LGA) not in State Category') 'LGA_Cat_Error',
CONCAT_WS(' | ', d.Status, IF(b.`address_hierarchy_entry_id`=c.`parent_id`,NULL, 'City/Village (LGA) not in State Category')) Address_Error
FROM `person_address` a
LEFT JOIN (select `address_hierarchy_entry_id`, `name` from `address_hierarchy_entry` where `level_id`=2) b ON a.`state_province`=b.`name`
LEFT JOIN (SELECT `parent_id`, `name` FROM `address_hierarchy_entry` WHERE `level_id`=3) c ON a.`city_village`=c.`name`
left join Address_Clean2 d on a.`person_id`=d.`person_id`;*/
DROP TABLE IF EXISTS Address_Clean3;
CREATE TEMPORARY TABLE Address_Clean3 AS
SELECT a.`person_id`,b.`address_hierarchy_entry_id` stateId, c.`name` LGA, c.`parent_id` LGAId,
CONCAT_WS(' | ', d.Status, IF(b.`address_hierarchy_entry_id`=c.`parent_id`,NULL, 'City/Village (LGA) not in State Category')) Address_Error
FROM `person_address` a
LEFT JOIN (SELECT `address_hierarchy_entry_id`, `name` FROM `address_hierarchy_entry` WHERE `level_id`=2) b ON a.`state_province`=b.`name`
LEFT JOIN (SELECT `parent_id`, `name` FROM `address_hierarchy_entry` WHERE `level_id`=3) c ON a.`city_village`=c.`name`
...
DROP TABLE IF EXISTS GISAT_reviwed;
CREATE TEMPORARY TABLE GISAT_reviwed AS
SELECT 'CIHP' IP, @State State, @SurgeCommand SurgeCommand, @LGA LGA, @FacilityName FacilityName, p.Patient_Id, p.`identifier` PEPID,
 a.`value_datetime` AS HIVEnroll_StartDate, b.`value_datetime` AS ARVCommence_StartDate, b.ARTRegAtStart,
 IF(p.`identifier_type` = 4 AND `preferred` != 1,'PEPID Not Set as Preferred', NULL) PEPID_Not_Preferred,
 IF(a.`value_datetime` != b.`value_datetime` AND b.ARTRegAtStart IS NOT NULL, 'ART Start Date Mismatch', NULL) ART_Start_Date,
 IF(b.ARTRegAtStart IS NULL, 'Missing ART Reg. at Start', NULL) ART_Reg_at_Start, 
IF(a.`value_datetime` IS NULL, 'No ART Start date on HIV Enrollment Form', NULL) HIV_Enrollment_Form,
IF(b.`value_datetime` IS NULL, 'No ART Start date on ART Commencement Form', NULL) ART_Commencement_Form,
IF((d.`value_datetime` IS NULL OR d.`value_datetime` = '' OR d.`value_datetime` LIKE '%000%'), 'Specify Date Confirmed HIV+',NULL) Date_Confirme_HIV, e.Address_Error
FROM `patient_identifier` p
LEFT JOIN HIVEnrollment_Cleaning a ON a.`person_id` = p.`patient_id` LEFT JOIN ARVCommencement_Cleaning b ON b.`person_id` = p.`patient_id` 
LEFT JOIN (SELECT o.`person_id`, o.`value_datetime` FROM obs o WHERE o.`concept_id` = 160554 AND o.`voided`=0) AS d ON d.person_id=p.`patient_id`
LEFT JOIN (SELECT * FROM Address_Clean3 WHERE Address_Error IS NOT NULL GROUP BY person_id,Address_Error) e ON p.`patient_id`=e.`person_id`
WHERE p.`identifier_type` = 4 AND p.`voided` = 0;

SELECT 'CIHP' IP, @State State, @SurgeCommand SurgeCommand, @LGA LGA, @FacilityName FacilityName, Patient_Id, PEPID,
HIVEnroll_StartDate,ARVCommence_StartDate, ARTRegAtStart,
CASE
WHEN CONCAT_WS(' | ', PEPID_Not_Preferred,ART_Start_Date,ART_Reg_at_Start,HIV_Enrollment_Form,ART_Commencement_Form,Date_Confirme_HIV,Address_Error)='' THEN 'Validation Completed'
ELSE CONCAT_WS(' | ', PEPID_Not_Preferred,ART_Start_Date,ART_Reg_at_Start,HIV_Enrollment_Form,ART_Commencement_Form,Date_Confirme_HIV,Address_Error) END AS Error_Type
 FROM GISAT_reviwed GROUP BY Patient_Id;

/*SELECT
  identifier,
  person_id,
  encounter_datetime,
  'No ART Start Date on HIV Enrollment Form' AS 'Error Type'
FROM
  Commencement_Cleaning
WHERE All_Concept_Ids NOT LIKE '%159599%'
ORDER BY encounter_datetime DESC;*/
 DROP TABLE IF EXISTS Pharm_Cleaning;
CREATE TEMPORARY TABLE Pharm_Cleaning AS
SELECT
  p.`identifier`,
  e.`encounter_id`,
  o.`person_id`,
...
SELECT 'CIHP' IP, @State State, @SurgeCommand SurgeCommand, @LGA LGA, @FacilityName FacilityName,
  ...
  person_id,
  encounter_datetime,
  'Empty Medication Duration' AS 'Error Type'
FROM
  Pharm_Cleaning
WHERE All_Concept_Ids NOT LIKE '%159368%'

UNION ALL 
SELECT 'CIHP' IP, @State State, @SurgeCommand SurgeCommand, @LGA LGA, @FacilityName FacilityName, `identifier`,
  `patient_id`,
  `encounter_datetime`,
  'Drug duration not specified for 3rd Reg' AS 'Error Type' FROM More_DrugDur_Cleaning 
  ORDER BY encounter_datetime DESC;
/*SELECT 'CIHP' IP, @State State, @SurgeCommand SurgeCommand, @LGA LGA, @FacilityName FacilityName, 
  identifier,
  person_id,
  encounter_datetime,
  'Empty Quantity of medication prescribed' AS 'Error Type'
FROM
  Pharm_Cleaning
WHERE All_Concept_Ids NOT LIKE '%160856%'
ORDER BY encounter_datetime DESC;*/

/*Cleaning Viral Load */
/*CIHP_ViralLoad_Data*/
DROP TABLE IF EXISTS CIHP_VLOcc;
CREATE TEMPORARY TABLE CIHP_VLOcc AS
SELECT @row_no := IF(@prev_val = e.patient_id , @row_no + 1, 1) AS Occurrence, @prev_val := e.patient_id AS person_id, e.`encounter_datetime`, e.`encounter_id`
FROM `encounter` e, (SELECT @row_no := 0) X, (SELECT @prev_val := '') Y
WHERE e.`encounter_type` = 11 AND e.`form_id` = 21 AND e.`voided` = 0 AND e.`visit_id` IS NOT NULL
ORDER BY e.patient_id, e.`encounter_datetime` DESC;

DROP TABLE IF EXISTS VL_Search; CREATE TEMPORARY TABLE VL_Search AS
SELECT a.person_id, p2.identifier Pepid, a.encounter_datetime, a.encounter_id, c.Viral_Load_Order, IF(c.Viral_Load_Order IS NULL, 'Specify Viral Load Test', NULL) Viral_Load_Order_Check,
d.PCR_Lab_Sample_No, IF(d.PCR_Lab_Sample_No IS NULL, 'Specify PCR Lab Sample No', NULL) PCR_Lab_Sample_No_Check, e.Specimen_sources, 
IF(e.Specimen_sources IS NULL, 'Specify Sample Type', NULL) Specimen_sources_Check, f.Indication_For_The_Viral_Load_Test, 
IF(f.Indication_For_The_Viral_Load_Test IS NULL, 'Specify Indication For Viral Load', NULL) Indication_Check,
c.Visit_Date, b.Sample_Collection_Date, CASE
WHEN b.Sample_Collection_Date < c.Visit_Date THEN 'Sample Collection Date Earlier than Visit Date'
WHEN b.Sample_Collection_Date > c.Visit_Date THEN 'Sample Collection Date / Visit Date Misatch (Please confirm)'
WHEN b.Sample_Collection_Date IS NULL THEN 'Specify Sample Collection Date'
ELSE NULL END AS Error_Type, o.CurrentVL, g.Date_Received_at_PCR_Lab, h.Date_result_was_received_at_the_facility
FROM CIHP_VLOcc a
LEFT JOIN (SELECT `person_id`, `encounter_id`, `value_numeric` CurrentVL FROM `obs` WHERE `concept_id` = 856 
AND voided = 0 GROUP BY `person_id`, `encounter_id`, `obs_datetime`) o ON  o.`person_id`=a.`person_id` AND o.`encounter_id`=a.`encounter_id` 
LEFT JOIN (SELECT person_id, `encounter_id`, `value_datetime` Sample_Collection_Date FROM `obs` WHERE `concept_id` = 159951 
AND voided = 0 GROUP BY `person_id`, `encounter_id`, `obs_datetime`) b ON b.`person_id`=a.`person_id` AND a.`encounter_id`=b.`encounter_id`
LEFT JOIN (SELECT person_id, `encounter_id`, `obs_datetime` Visit_Date,`value_coded` Viral_Load_Order FROM `obs` WHERE `concept_id` = 165765 
AND voided = 0 GROUP BY `person_id`, `encounter_id`, `obs_datetime`) c ON c.`person_id`=a.`person_id` AND a.`encounter_id`=c.`encounter_id`
LEFT JOIN (SELECT person_id, `encounter_id`, `value_text` PCR_Lab_Sample_No FROM `obs` WHERE `concept_id` = 165715 
AND voided = 0 GROUP BY `person_id`, `encounter_id`, `obs_datetime`) d ON d.`person_id`=a.`person_id` AND a.`encounter_id`=d.`encounter_id`
LEFT JOIN (SELECT person_id, `encounter_id`, `value_coded` Specimen_sources FROM `obs` WHERE `concept_id` = 162476 
AND voided = 0 GROUP BY `person_id`, `encounter_id`, `obs_datetime`) e ON e.`person_id`=a.`person_id` AND a.`encounter_id`=e.`encounter_id`
LEFT JOIN (SELECT person_id, `encounter_id`, `value_coded` Indication_For_The_Viral_Load_Test FROM `obs` WHERE `concept_id` = 164980 
AND voided = 0 GROUP BY `person_id`, `encounter_id`, `obs_datetime`) f ON f.`person_id`=a.`person_id` AND a.`encounter_id`=f.`encounter_id`
LEFT JOIN (SELECT person_id, `encounter_id`, `value_datetime` Date_Received_at_PCR_Lab FROM `obs` WHERE `concept_id` = 165716 
AND voided = 0 GROUP BY `person_id`, `encounter_id`, `obs_datetime`) g ON g.`person_id`=a.`person_id` AND a.`encounter_id`=g.`encounter_id`
LEFT JOIN (SELECT person_id, `encounter_id`, `value_datetime` Date_result_was_received_at_the_facility FROM `obs` WHERE `concept_id` = 165987 
AND voided = 0 GROUP BY `person_id`, `encounter_id`, `obs_datetime`) h ON h.`person_id`=a.`person_id` AND a.`encounter_id`=h.`encounter_id`
LEFT JOIN (SELECT person_id, `encounter_id`, `value_datetime` Ordered_date FROM `obs` WHERE `concept_id` = 164989 
AND voided = 0 GROUP BY `person_id`, `encounter_id`, `obs_datetime`) i ON i.`person_id`=a.`person_id` AND a.`encounter_id`=i.`encounter_id`
LEFT JOIN `patient_identifier` p2 ON a.`person_id` = p2.`patient_id` AND p2.`identifier_type` = 4;
SELECT  'CIHP' IP, @State State, @SurgeCommand SurgeCommand, @LGA LGA, @FacilityName FacilityName, Pepid, Encounter_Datetime, Visit_Date, Sample_Collection_Date, CurrentVL,
IF(CONCAT_WS(" | ", Viral_Load_Order_Check, PCR_Lab_Sample_No_Check, Specimen_sources_Check, Specimen_sources_Check, Indication_Check,Error_Type)='', 'Lab Form Validation Completed',
CONCAT_WS(" | ", Viral_Load_Order_Check, PCR_Lab_Sample_No_Check, Specimen_sources_Check, Specimen_sources_Check, Indication_Check,Error_Type)) Error
FROM VL_Search GROUP BY Pepid, Encounter_Datetime, Visit_Date ORDER BY Encounter_Datetime DESC;

SELECT 'CIHP' IP, @State State, @SurgeCommand SurgeCommand, @LGA LGA, @FacilityName FacilityName,
  identifier,
  person_id,
  encounter_datetime,
  'Empty Quantity of medication Dispensed' AS 'Error Type'
FROM
  Pharm_Cleaning
WHERE All_Concept_Ids NOT LIKE '%1443%'
ORDER BY encounter_datetime DESC;
   DROP TABLE IF EXISTS RegimenLineNotSelected;
CREATE TEMPORARY TABLE RegimenLineNotSelected AS
  SELECT
  identifier,
  person_id,
  MAX(encounter_datetime) AS VisitDate,
  'Regimen Line Not Selected' AS Error_Type
FROM
  Pharm_Cleaning
WHERE All_Concept_Ids NOT LIKE '%165708%'  GROUP BY identifier
ORDER BY encounter_datetime DESC;


   DROP TABLE IF EXISTS ARVRegimenNotSpecified;
CREATE TEMPORARY TABLE ARVRegimenNotSpecified AS
  SELECT
  identifier,
  person_id,
  MAX(encounter_datetime) AS VisitDate,
  'ARV regimen not Specified' AS Error_Type
FROM
  Pharm_Cleaning
WHERE All_Concept_Ids NOT LIKE '%164506%' AND 
All_Concept_Ids NOT LIKE '%164507%' AND
All_Concept_Ids NOT LIKE '%164513%' AND
All_Concept_Ids NOT LIKE '%164514%' AND
All_Concept_Ids NOT LIKE '%165702%' AND
All_Concept_Ids NOT LIKE '%165703%'
GROUP BY identifier
ORDER BY encounter_datetime DESC;
SELECT 'CIHP' IP, @State State, @SurgeCommand SurgeCommand, @LGA LGA, @FacilityName FacilityName, 
identifier, person_id, VisitDate, Error_Type FROM RegimenLineNotSelected
UNION ALL
SELECT 'CIHP' IP, @State State, @SurgeCommand SurgeCommand, @LGA LGA, @FacilityName FacilityName, 
identifier, person_id, VisitDate, Error_Type  FROM ARVRegimenNotSpecified ORDER BY VisitDate DESC;


SELECT 'CIHP' IP, @State State, @SurgeCommand SurgeCommand, @LGA LGA, @FacilityName FacilityName,
  o.`person_id`,
  p.`identifier`,
  o.`obs_datetime` AS VisitDate,
  o.`concept_id`,
  o.`value_numeric`,
  'Quantity above 180' AS 'Error_type'
FROM
  obs o
  LEFT JOIN `patient_identifier` p
    ON o.`person_id` = p.`patient_id`
WHERE o.`concept_id` = '159368'
  AND `value_numeric` > 180
  AND p.`identifier_type` = 4
  ...
  o.`concept_id`,
  o.`value_numeric`,
  'Quantity lessthan 14 Days' AS 'Error_type'
FROM
  obs o
  LEFT JOIN `patient_identifier` p
    ON o.`person_id` = p.`patient_id`
WHERE o.`concept_id` = '159368'
  AND `value_numeric` < 14
  AND p.`identifier_type` = 4
  AND o.`obs_datetime` <= CURDATE()
  AND o.`voided` = 0
  ORDER BY o.`obs_datetime` DESC;
  
DROP TABLE IF EXISTS GISAT_PharmPregStatus;
CREATE TEMPORARY TABLE GISAT_PharmPregStatus AS
SELECT @row_no := IF(@prev_val = o.person_id , @row_no + 1, 1) AS Occurrence, @prev_val := o.person_id AS person_id, p2.`identifier` Pepid,
...
LEFT JOIN `person` p ON p.`person_id`=o.`person_id` AND p.`voided`=0,
(SELECT @row_no := 0) X, (SELECT @prev_val := '') Y 
WHERE e.`encounter_type` = 13 AND e.`form_id` = 27 AND o.`concept_id` = 165050 AND o.`voided` = 0 
ORDER BY o.person_id, o.`obs_datetime` DESC;
DROP TABLE IF EXISTS PregStatus_Comp;
CREATE TEMPORARY TABLE PregStatus_Comp AS
SELECT gp.*, gc.obs_datetime Carecard_VisitDate, gc.PregStatus CareCard_PregStatus FROM GISAT_PharmPregStatus gp LEFT JOIN /*pregnancy status from Care Care*/(
SELECT @row_no := IF(@prev_val = o.person_id , @row_no + 1, 1) AS Occurrence, @prev_val := o.person_id AS person_id, p2.`identifier` Pepid,
p.`gender`, o.`concept_id`, o.`obs_datetime`, o.`value_coded`, c2.`name` PregStatus  FROM `obs` o 
LEFT JOIN `patient_identifier` p2 ON o.`person_id` = p2.`patient_id` AND p2.`identifier_type` = 4 AND p2.`preferred` = 1
LEFT JOIN `concept_name` c2 ON o.`value_coded`=c2.`concept_id` AND c2.locale = 'en' AND c2.locale_preferred = 1
...
ORDER BY o.person_id, o.`obs_datetime` DESC) AS gc ON gp.person_id=gc.person_id AND gp.obs_datetime=gc.obs_datetime
WHERE gp.Occurrence = 1 AND gp.gender = "F" ORDER BY gp.obs_datetime DESC;
SELECT 'CIHP' IP, @State State, @SurgeCommand SurgeCommand, @LGA LGA, @FacilityName FacilityName,
person_id, pepid, gender Sex, obs_datetime VisitDate, PregStatus Pharmacy_PregStatus, Carecard_PregStatus,
CASE WHEN PregStatus = Carecard_PregStatus THEN 'Preg. Status Validated'
WHEN PregStatus IN ('Not Pregnant', 'Pregnant', 'Breastfeeding') AND Carecard_PregStatus IS NULL THEN 'No corresponding Care card entry'
WHEN PregStatus != Carecard_PregStatus THEN 'Preg. Status Mismatch'
ELSE ''
END AS 'Validation Status'
FROM PregStatus_Comp ORDER BY obs_datetime DESC;

  END
""")

cur.execute("""DROP PROCEDURE IF EXISTS `Quick_CIHP_LineList`""")
cur.execute("""
  CREATE PROCEDURE Quick_CIHP_LineList()
  BEGIN

DROP TABLE IF EXISTS `validation_linelist`.Latest_IDs;
CREATE TABLE `validation_linelist`.Latest_IDs AS
SELECT `patient_id` AS Latest_ID_for_Quick_List FROM `openmrs`.`encounter` 
WHERE encounter_id >= (SELECT `Last_Enc_id` 
FROM `validation_linelist`.`last_encounter_id`) OR encounter_id IN (SELECT `encounter_id` 
FROM `openmrs`.`encounter` WHERE `date_changed` >= (SELECT `Last_Date_time` FROM `validation_linelist`.`last_encounter_id`)) GROUP BY patient_id;

DROP TABLE IF EXISTS `openmrs`.Latest_IDs;
CREATE TABLE `openmrs`.Latest_IDs AS
SELECT `patient_id` AS Latest_ID_for_Quick_List FROM `openmrs`.`encounter` 
WHERE encounter_id >= (SELECT `Last_Enc_id` 
FROM `validation_linelist`.`last_encounter_id`) OR encounter_id IN (SELECT `encounter_id` 
FROM `openmrs`.`encounter` WHERE `date_changed` >= (SELECT `Last_Date_time` FROM `validation_linelist`.`last_encounter_id`)) GROUP BY patient_id;
  
#SET @Reporting_Date := '2025-01-08';
SET @ValidVLDate1 := (@Reporting_Date) - INTERVAL (SELECT DAYOFYEAR(CONCAT(YEAR(@Reporting_Date),'-12-31'))) DAY;
DROP TABLE IF EXISTS CIHP_listoffacility;
CREATE TABLE IF NOT EXISTS  CIHP_listoffacility (
  `FacilityID` DOUBLE DEFAULT NULL,
...
LEFT JOIN `encounter` en ON a.`patient_id`=en.`patient_id`
LEFT JOIN Biometrics_Captured g ON a.`patient_id`=g.`patient_id`
WHERE en.`encounter_type` = 14 AND en.`encounter_datetime` <= @Reporting_Date AND en.`voided`=0 AND a.`patient_id` IN (SELECT * FROM Latest_IDs)
AND a.`voided`=0 AND b.`identifier_type` = 4
GROUP BY a.`patient_id`;
SET SESSION sql_mode = '';
DROP TABLE IF EXISTS CIHP_Hop;
...
SELECT a.`patient_id`, a.`identifier` HospitalNo
FROM `patient_identifier` a
WHERE a.`voided`=0 AND a.`identifier_type` = 5 AND a.`patient_id` IN (SELECT * FROM Latest_IDs)
GROUP BY a.`patient_id`;
DROP TABLE IF EXISTS CIHP_ARTStartDate;
CREATE TEMPORARY TABLE CIHP_ARTStartDate AS
SELECT o.`person_id`, o.`value_datetime` AS ARTStartDate
FROM `obs` o LEFT JOIN `encounter` e ON o.`encounter_id`=e.`encounter_id` 
WHERE o.`concept_id` = 159599 AND e.`encounter_type` = 25 AND e.`form_id` = 56 AND o.`voided`=0 AND o.`value_datetime` <= @Reporting_Date AND o.`person_id` IN (SELECT * FROM Latest_IDs);
/*First CurrentRegimenLine*/
DROP TABLE IF EXISTS CIHP_1st_RegimenLine;
CREATE TEMPORARY TABLE CIHP_1st_RegimenLine AS
SELECT o.`person_id`, o.`concept_id`, o.`obs_datetime`, o.`value_coded`,ConceptName(o.`value_coded`) RegLineAtStart  FROM `obs` o
LEFT JOIN `encounter` e ON o.`encounter_id`=e.`encounter_id`
WHERE e.`encounter_type` = 25 AND e.`form_id` = 56 AND o.`concept_id` = 165708 AND o.`voided` = 0 AND o.`person_id` IN (SELECT * FROM Latest_IDs)
GROUP BY o.`person_id`, o.`obs_datetime`;
/*First ARV_Medication*/
DROP TABLE IF EXISTS CIHP_1st_ARV_Medication;
CREATE TEMPORARY TABLE CIHP_1st_ARV_Medication AS
SELECT o.`person_id`, o.`concept_id`, o.`obs_datetime`, o.`value_coded`, ConceptName(o.`value_coded`) ARTRegAtStart  FROM `obs` o 
LEFT JOIN `encounter` e ON o.`encounter_id`=e.`encounter_id`
WHERE e.`encounter_type` = 25 AND e.`form_id` = 56 AND o.`concept_id` IN (164506, 164507, 164513, 164514, 165702, 165703) AND o.`voided` = 0 AND o.`person_id` IN (SELECT * FROM Latest_IDs)
GROUP BY o.`person_id`, o.`obs_datetime`;
/*SELECT o.`person_id`, o.`concept_id`, c.`name`, o.`obs_datetime` VisitDate, o.`value_coded`, c2.`name`, o.`value_datetime`, o.`value_coded_name_id`, o.`value_numeric`
FROM `obs` o LEFT JOIN `encounter` e ON o.`encounter_id`=e.`encounter_id`
left join `concept_name` c on o.`concept_id`=c.`concept_id` AND c.locale = 'en' AND c.locale_preferred = 1 
...
SELECT a.patient_id, a. LastPickupDate, a.`encounter_id`, b.DaysOfARVRefill, c.Pill_Balance, d.Next_Pickup_Date, e.CurrentARTReg, n.CurrentRegLine, /*g.ARV_Drug_Strength,
h.OI_Drug_CTX, j.CTX_Strength, i.OI_Drug_INH, k.INH_Strength, l.DSD_Model, m.DDD_Fac_Disp,*/ o.PregStatus
FROM (SELECT e.patient_id, e.encounter_datetime AS LastPickupDate, e.encounter_id, @row_num := IF(@prev_patient = e.patient_id, @row_num + 1, 1) AS Occ, @prev_patient := e.patient_id
FROM (SELECT e.patient_id, e.encounter_datetime, e.encounter_id
     FROM encounter e
     WHERE e.encounter_type = 13 AND e.form_id = 27 AND e.voided = 0 AND e.encounter_datetime <= @Reporting_Date AND e.patient_id IN (SELECT * FROM Latest_IDs)
     ORDER BY e.patient_id, e.encounter_datetime DESC) e, 
    (SELECT @row_num := 0, @prev_patient := NULL) vars
HAVING Occ = 1) a
#ART Refill Duration
LEFT JOIN (SELECT o.`person_id`, o.encounter_id, o.`obs_id`, o.`value_numeric` DaysOfARVRefill FROM `obs` o WHERE o.concept_id = 159368 AND o.`voided` = 0 AND o.`person_id` IN (SELECT * FROM Latest_IDs)
AND obs_group_id IN (SELECT obs_id FROM obs WHERE concept_id = 162240 AND `voided` = 0) GROUP BY o.encounter_id) b ON a.encounter_id = b.encounter_id 
#Pill Balance Field
LEFT JOIN (SELECT person_id, CAST(`value_text` AS UNSIGNED) Pill_Balance, encounter_id FROM obs 
WHERE concept_id = 166406 AND `voided` = 0 AND `person_id` IN (SELECT * FROM Latest_IDs)) c ON c.encounter_id=a.encounter_id
#Pharm Next Pickup
LEFT JOIN (SELECT person_id, `value_datetime` Next_Pickup_Date, encounter_id FROM obs WHERE concept_id = 5096 AND `voided` = 0 AND `person_id` IN (SELECT * FROM Latest_IDs)) d ON d.encounter_id=a.encounter_id
/*Last ARV_Medication*/
LEFT JOIN (SELECT o.encounter_id, o.`value_coded`, ConceptName(o.`value_coded`) CurrentARTReg FROM `obs` o 
WHERE `concept_id` IN (164506, 164507, 164513, 164514, 165702, 165703) AND o.`voided` = 0 AND obs_group_id IS NULL AND o.`person_id` IN (SELECT * FROM Latest_IDs)) e ON e.encounter_id=a.encounter_id
/*Last CurrentRegimenLine*/
LEFT JOIN (SELECT o.`person_id`, o.`encounter_id`, o.`obs_datetime`, ConceptName(o.`value_coded`) CurrentRegLine  FROM `obs` o
WHERE o.`concept_id` = 165708 AND o.`voided` = 0 AND o.`person_id` IN (SELECT * FROM Latest_IDs)) f ON f.encounter_id=a.encounter_id
/*#ARV Drug Strength
LEFT JOIN (SELECT o.`person_id`, o.`encounter_id`, o.`obs_id`, GROUP_CONCAT(DISTINCT ConceptName(o.`value_coded`) SEPARATOR '-') ARV_Drug_Strength FROM `obs` o
WHERE o.concept_id = 165725 AND o.`voided` = 0 AND obs_group_id IN (SELECT obs_id FROM obs WHERE concept_id = 162240 AND `voided` = 0) 
GROUP BY o.`encounter_id`) g ON g.encounter_id=a.encounter_id
...
/*Last CurrentRegimenLine*/
LEFT JOIN (SELECT o.`person_id`, o.`encounter_id`, o.`obs_datetime`, o.`value_coded`, ConceptName(o.`value_coded`) CurrentRegLine  FROM `obs` o 
WHERE o.`concept_id` = 165708 AND o.`voided` = 0 AND o.`person_id` IN (SELECT * FROM Latest_IDs)) n ON n.encounter_id=a.encounter_id
/*CIHP_PregStatus*/
LEFT JOIN (SELECT o.person_id, p.`gender`, o.encounter_id, ConceptName(o.`value_coded`) PregStatus FROM `obs` o
LEFT JOIN `person` p ON p.`person_id`=o.`person_id` AND p.`voided`=0
WHERE o.`concept_id` = 165050 AND o.`voided` = 0 AND p.gender = 'F' AND o.`person_id` IN (SELECT * FROM Latest_IDs)) o ON o.encounter_id=a.encounter_id
WHERE a.Occ = 1;
...
/*CIHP_ViralLoad_Data*/
DROP TABLE IF EXISTS CIHP_ViralLoad_Data;CREATE TEMPORARY TABLE CIHP_ViralLoad_Data AS
SELECT @row_no := IF(@prev_val = w.patient_id , @row_no + 1, 1) AS Occurrence, @prev_val := w.patient_id AS person_id, w.`identifier` Pepid,
w.`encounter_datetime` DateOfCurrentVL, w.`value_numeric` CurrentVL, Date_Result_Received
FROM (SELECT e.patient_id, p2.`identifier`, e.`encounter_datetime`, o.`value_numeric`, d.Date_Result_Received 
FROM `obs` o LEFT JOIN `encounter` e ON o.`encounter_id`=e.`encounter_id`
LEFT JOIN `patient_identifier` p2 ON o.`person_id` = p2.`patient_id` AND p2.`identifier_type` = 4
LEFT JOIN (SELECT DISTINCT person_id, encounter_id, value_datetime AS Date_Result_Received FROM obs WHERE concept_id = 165987 AND voided=0 AND `person_id` IN (SELECT * FROM Latest_IDs))
AS d ON  o.person_id=d.person_id AND o.`encounter_id`=d.`encounter_id`
WHERE o.`concept_id` = 856 AND e.`encounter_type` = 11 AND e.`form_id` = 21  AND o.`voided` = 0 AND e.`encounter_datetime` <= @Reporting_Date AND o.`person_id` IN (SELECT * FROM Latest_IDs)
ORDER BY e.patient_id, e.`encounter_datetime` DESC) w,
(SELECT @row_no := 0, @prev_patient := NULL) vars
HAVING Occurrence = 1; 
...
LEFT JOIN `encounter` e ON o.`encounter_id`=e.`encounter_id`
LEFT JOIN VL_Search a ON o.`person_id`=a.`person_id` AND o.`obs_datetime`=a.DateOfCurrentVL
WHERE e.`encounter_type` = 11 AND e.`form_id` = 21 AND o.`concept_id` = 164980 AND o.`voided` = 0 AND  o.`person_id`= a.`person_id` AND o.`person_id` IN (SELECT * FROM Latest_IDs)
GROUP BY o.`person_id`, o.`obs_datetime`;
/*CIHP_Transfer_In*/
DROP TABLE IF EXISTS CIHP_Transfer_In;
CREATE TEMPORARY TABLE CIHP_Transfer_In AS
SELECT * FROM (SELECT o.`person_id`, o.`concept_id`, o.`obs_datetime`, o.`value_coded`, ConceptName(o.`value_coded`) TI  FROM `obs` o 
LEFT JOIN `encounter` e ON o.`encounter_id`=e.`encounter_id`
WHERE e.`encounter_type` = 14 AND e.`form_id` = 23 AND o.`concept_id` = 160540 AND o.`voided` = 0  AND o.`obs_datetime` <= @Reporting_Date AND o.`person_id` IN (SELECT * FROM Latest_IDs)
GROUP BY o.`person_id`, o.`obs_datetime`) a WHERE a.TI LIKE 'Trans%';
/*Create Latest Weight
DROP TABLE IF EXISTS Last_Weight;
CREATE TEMPORARY TABLE Last_Weight AS
...
DROP TABLE IF EXISTS All_Weight;
CREATE TEMPORARY TABLE All_Weight AS
SELECT @row_no := IF(@prev_val = a.person_id , @row_no + 1, 1) AS Occurrence, @prev_val := a.person_id AS person_id, a.`identifier` Pepid,
 a.obs_datetime AS Visit_Date, a.value_numeric AS Weight
FROM (SELECT o.person_id, p2.`identifier`, o.obs_datetime, o.value_numeric FROM `obs` o 
LEFT JOIN `patient_identifier` p2 ON o.`person_id` = p2.`patient_id` AND p2.`identifier_type` = 4
WHERE o.concept_id = 5089 AND p2.`identifier_type`=4  AND o.`voided` = 0 AND o.`obs_datetime` <= @Reporting_Date AND o.`person_id` IN (SELECT * FROM Latest_IDs)
...
DROP TABLE IF EXISTS CIHP_VL_SampleCol1;
CREATE TEMPORARY TABLE CIHP_VL_SampleCol1 AS
SELECT a.person_id, a.Last_VL_Sample_Date, a.encounter_id, a.Occ AS Occurrence, a.obs_datetime
FROM (SELECT e.person_id, e.Last_VL_Sample_Date, e.encounter_id, @row_num := IF(@prev_patient = e.person_id, @row_num + 1, 1) AS Occ, @prev_patient := e.person_id, e.obs_datetime
FROM (SELECT o.person_id, o.encounter_id, `obs_datetime`, value_datetime AS Last_VL_Sample_Date FROM obs o
LEFT JOIN `encounter` e ON o.`encounter_id`=e.`encounter_id`
WHERE o.concept_id = 159951 AND o.voided=0 AND e.`encounter_type` = 11 AND e.`form_id` = 21 AND o.`obs_datetime` <= @Reporting_Date AND e.patient_id IN (SELECT * FROM Latest_IDs)
ORDER BY o.person_id, o.`obs_datetime` DESC) e,
(SELECT @row_num := 0, @prev_patient := NULL) vars
HAVING Occ = 1) a;
DROP TABLE IF EXISTS CIHP_VL_SampleCol;
CREATE TEMPORARY TABLE CIHP_VL_SampleCol AS
...
FROM `encounter` e, #LEFT JOIN obs o ON o.`encounter_id`=e.`encounter_id`,
(SELECT @row_no := 0) X, (SELECT @prev_val := '') Y
WHERE  e.`encounter_type` = 15 AND e.`form_id` = 13  AND e.`voided` = 0 AND e.`encounter_datetime` <= @Reporting_Date AND e.`patient_id` IN (SELECT * FROM Latest_IDs)
ORDER BY e.patient_id, e.`encounter_datetime` DESC) a
LEFT JOIN (SELECT `person_id`, ConceptName(`value_coded`) AS 'Reason FOR Tracking', `obs_datetime`, o.encounter_id FROM obs o 
WHERE o.concept_id = 165460  AND o.voided=0 AND `person_id` IN (SELECT * FROM Latest_IDs)) b
ON a.person_id=b.person_id AND a.encounter_id=b.encounter_id
LEFT JOIN (SELECT `person_id`, `value_text` AS 'Partner_full_name', `obs_datetime`, o.encounter_id FROM obs o WHERE concept_id = 161135 AND o.voided=0 AND `person_id` IN (SELECT * FROM Latest_IDs)) c 
ON a.person_id=c.person_id AND a.encounter_id=c.encounter_id
LEFT JOIN (SELECT `person_id`, `value_text` AS 'Address_of_Tx_supporter', `obs_datetime`, o.`encounter_id` FROM obs o WHERE concept_id = 160641 AND o.voided=0 AND `person_id` IN (SELECT * FROM Latest_IDs)) d
ON a.person_id=d.person_id AND a.encounter_id=d.encounter_id
LEFT JOIN (SELECT `person_id`, `value_text` AS 'Contact_phone_No', `obs_datetime`, o.`encounter_id` FROM obs o WHERE concept_id = 159635 AND o.voided=0 AND `person_id` IN (SELECT * FROM Latest_IDs)) e
ON a.person_id=e.person_id AND a.encounter_id=e.encounter_id
LEFT JOIN (SELECT `person_id`, `value_datetime` AS 'Date_of_Last_Actual', `obs_datetime`, o.`encounter_id` FROM obs o WHERE concept_id = 165461 AND o.voided=0 AND `person_id` IN (SELECT * FROM Latest_IDs)) f
ON a.person_id=f.person_id AND a.encounter_id=f.encounter_id
LEFT JOIN (SELECT `person_id`, `value_datetime` AS 'Date_Missed_Scheduled_App', `obs_datetime`, o.`encounter_id` FROM obs o WHERE concept_id = 165778 AND o.voided=0 AND `person_id` IN (SELECT * FROM Latest_IDs)) g
ON a.person_id=g.person_id AND a.encounter_id=g.encounter_id
LEFT JOIN (SELECT `person_id`, `value_datetime` AS 'ContAttempt_Date1', MAX(`obs_datetime`) AS obs_datetime, o.`encounter_id` FROM obs o
WHERE concept_id = 165463 AND o.voided = 0 AND `obs_group_id` IS NOT NULL AND `person_id` IN (SELECT * FROM Latest_IDs) GROUP BY person_id, obs_datetime) h
ON a.person_id=h.person_id AND a.encounter_id=h.encounter_id
LEFT JOIN (SELECT `person_id`, `value_text` AS 'who_attempted_contact1', MAX(`obs_datetime`) AS obs_datetime, o.`encounter_id` FROM obs o 
WHERE concept_id = 165464 AND o.voided = 0 AND `obs_group_id` IS NOT NULL AND `person_id` IN (SELECT * FROM Latest_IDs) GROUP BY person_id, obs_datetime) i
ON a.person_id=i.person_id AND a.encounter_id=i.encounter_id
LEFT JOIN (SELECT `person_id`, ConceptName(`value_coded`) AS 'Mode_of_Communication1', MAX(`obs_datetime`) AS obs_datetime,  o.`encounter_id` FROM obs o
WHERE concept_id = 165465 AND o.voided = 0 AND `obs_group_id` IS NOT NULL AND `person_id` IN (SELECT * FROM Latest_IDs) GROUP BY person_id, obs_datetime) j
ON a.person_id=j.person_id AND a.encounter_id=j.encounter_id
LEFT JOIN (SELECT `person_id`, ConceptName(`value_coded`) AS 'Person_Contacted1', MAX(`obs_datetime`) AS obs_datetime, o.`encounter_id` FROM obs o 
WHERE concept_id = 165466 AND o.voided=0 AND `obs_group_id` IS NOT NULL AND `person_id` IN (SELECT * FROM Latest_IDs) GROUP BY person_id, obs_datetime) k
ON a.person_id=k.person_id AND a.encounter_id=k.encounter_id
LEFT JOIN(SELECT `person_id`, ConceptName(`value_coded`) AS 'Reason_for_Defaulting1', MAX(`obs_datetime`) AS obs_datetime, o.`encounter_id` FROM obs o 
WHERE concept_id = 165467 AND o.voided=0 AND `obs_group_id` IS NOT NULL AND `person_id` IN (SELECT * FROM Latest_IDs) GROUP BY person_id, obs_datetime) l
ON a.person_id=l.person_id AND a.encounter_id=l.encounter_id
LEFT JOIN (SELECT `person_id`, ConceptName(`value_coded`) AS 'Reason_for_Termination', `obs_datetime`, o.`encounter_id` FROM obs o WHERE concept_id = 165470 AND o.voided=0 AND `person_id` IN (SELECT * FROM Latest_IDs)) m
ON a.person_id=m.person_id AND a.encounter_id=m.encounter_id
LEFT JOIN (SELECT `person_id`, `value_datetime` AS 'Date_of_Termination', `obs_datetime`, o.`encounter_id` FROM obs o WHERE concept_id = 165469 AND o.voided=0 AND `person_id` IN (SELECT * FROM Latest_IDs)) n
ON a.person_id=m.person_id AND a.encounter_id=n.encounter_id
LEFT JOIN (SELECT `person_id`, ConceptName(`value_coded`) AS 'Previous_ARV_exposure', `obs_datetime`, o.`encounter_id` FROM obs o 
WHERE concept_id = 165586 AND o.voided=0 AND `person_id` IN (SELECT * FROM Latest_IDs)) o
ON  a.person_id=m.person_id AND a.encounter_id=o.encounter_id
LEFT JOIN (SELECT `person_id`, ConceptName(`value_coded`) AS 'Referred_for', `obs_datetime`, o.`encounter_id` FROM obs o WHERE concept_id = 165776 AND o.voided=0 AND `person_id` IN (SELECT * FROM Latest_IDs)) p
ON  a.person_id=m.person_id AND a.encounter_id=p.encounter_id
LEFT JOIN (SELECT `person_id`, `value_datetime` AS 'Date_Returned', `obs_datetime`, o.`encounter_id` FROM obs o WHERE concept_id = 165775 AND o.voided=0 AND `person_id` IN (SELECT * FROM Latest_IDs)) q
ON  a.person_id=q.person_id AND a.encounter_id=q.encounter_id
LEFT JOIN (SELECT `person_id`, `value_datetime` AS 'Date_LTFU', `obs_datetime`,  o.`encounter_id` FROM obs o WHERE concept_id = 166152 AND o.voided=0 AND `person_id` IN (SELECT * FROM Latest_IDs)) r
ON  a.person_id=m.person_id AND a.encounter_id=r.encounter_id
LEFT JOIN (SELECT `person_id`, ConceptName(`value_coded`) AS 'Reason_LTFU', `obs_datetime`, o.`encounter_id` FROM obs o WHERE concept_id = 166157 AND o.voided=0 AND `person_id` IN (SELECT * FROM Latest_IDs)) s
ON  a.person_id=m.person_id AND a.encounter_id=s.encounter_id
LEFT JOIN (SELECT `person_id`, ConceptName(`value_coded`) AS 'LTFU', `obs_datetime`, o.`encounter_id` FROM obs o WHERE concept_id = 5240 AND o.voided=0 AND `person_id` IN (SELECT * FROM Latest_IDs)) t
ON  a.person_id=m.person_id AND a.encounter_id=t.encounter_id
LEFT JOIN (SELECT `person_id`, `value_text` AS 'Name_oF_Tracker', `obs_datetime`, o.`encounter_id` FROM obs o WHERE concept_id = 165459 AND o.voided=0 AND `person_id` IN (SELECT * FROM Latest_IDs)) u
ON  a.person_id=m.person_id AND a.encounter_id=u.encounter_id
LEFT JOIN (SELECT `person_id`, `value_datetime` AS 'Tracker_Sig_Date', `obs_datetime`, o.`encounter_id` FROM obs o WHERE concept_id = 165777 AND o.voided=0 AND `person_id` IN (SELECT * FROM Latest_IDs)) v
ON  a.person_id=m.person_id AND a.encounter_id=v.encounter_id
LEFT JOIN (SELECT `person_id`, `value_text` AS 'Facility_Transferred_To', `obs_datetime`, o.`encounter_id` FROM obs o WHERE concept_id = 159495 AND o.voided=0 AND `person_id` IN (SELECT * FROM Latest_IDs)) w
ON  a.person_id=m.person_id AND a.encounter_id=w.encounter_id
WHERE a.Occurrence=1;
SET SESSION sql_mode = '';
DROP TABLE IF EXISTS All_IPT;
CREATE TEMPORARY TABLE All_IPT AS
  SELECT `person_id`, MAX(`obs_datetime`) obs_datetime FROM obs o 
  LEFT JOIN `encounter` e ON o.`encounter_id`=e.`encounter_id` WHERE e.`encounter_type` = 23 AND e.`form_id` = 53 AND o.`voided`= 0 AND o.`obs_datetime` <= @Reporting_Date 
  AND `person_id` IN (SELECT * FROM Latest_IDs)
  GROUP BY person_id;
DROP TABLE IF EXISTS Eligible_For_IPT;
CREATE TEMPORARY TABLE Eligible_For_IPT AS
  SELECT `person_id`, `value_coded` AS 'Eligible_For_IPT', `obs_datetime` FROM obs o
  LEFT JOIN `encounter` e ON o.`encounter_id`=e.`encounter_id` WHERE concept_id = 165986 AND e.`encounter_type` = 23 AND e.`form_id` = 53  AND o.`voided`= 0 AND `person_id` IN (SELECT * FROM Latest_IDs);
DROP TABLE IF EXISTS Date_IPT_start;
CREATE TEMPORARY TABLE Date_IPT_start AS
  SELECT `person_id`, `value_datetime` AS 'Date_IPT_start', `obs_datetime` FROM obs o
  LEFT JOIN `encounter` e ON o.`encounter_id`=e.`encounter_id` WHERE concept_id = 165994 AND e.`encounter_type` = 23 AND e.`form_id` = 53  AND o.`voided`= 0 AND `person_id` IN (SELECT * FROM Latest_IDs);
DROP TABLE IF EXISTS Outcome_of_IPT;
CREATE TEMPORARY TABLE Outcome_of_IPT AS
  SELECT `person_id`, `value_coded` AS 'Outcome_of_IPT', `obs_datetime` FROM obs o
  LEFT JOIN `encounter` e ON o.`encounter_id`=e.`encounter_id` WHERE concept_id = 166007 AND e.`encounter_type` = 23 AND e.`form_id` = 53  AND o.`voided`= 0 AND `person_id` IN (SELECT * FROM Latest_IDs);
DROP TABLE IF EXISTS Date_of_Outcome;
CREATE TEMPORARY TABLE Date_of_Outcome AS
  SELECT `person_id`, `value_datetime` AS 'Date_of_Outcome', `obs_datetime` FROM obs o
  LEFT JOIN `encounter` e ON o.`encounter_id`=e.`encounter_id` WHERE concept_id = 166008 AND e.`encounter_type` = 23 AND e.`form_id` = 53  AND o.`voided`= 0 AND `person_id` IN (SELECT * FROM Latest_IDs);
DROP TABLE IF EXISTS Sputum_AFB_TB;
CREATE TEMPORARY TABLE Sputum_AFB_TB AS
  SELECT `person_id`, IF(`value_coded` = 1, 'Sputum_AFB_TB', 'No') AS 'Sputum_AFB_TB', `obs_datetime` FROM obs o
  LEFT JOIN `encounter` e ON o.`encounter_id`=e.`encounter_id` WHERE concept_id = 166141 AND e.`encounter_type` = 23 AND e.`form_id` = 53  AND o.`voided`= 0 AND `person_id` IN (SELECT * FROM Latest_IDs);
DROP TABLE IF EXISTS GeneXpert_TB;
CREATE TEMPORARY TABLE GeneXpert_TB AS
  SELECT `person_id`, IF(`value_coded` = 1, 'GeneXpert_TB', 'No') AS 'GeneXpert_TB', `obs_datetime` FROM obs o
  LEFT JOIN `encounter` e ON o.`encounter_id`=e.`encounter_id` WHERE concept_id = 166142 AND e.`encounter_type` = 23 AND e.`form_id` = 53  AND o.`voided`= 0 AND `person_id` IN (SELECT * FROM Latest_IDs);
DROP TABLE IF EXISTS Chest_Xray_TB;
CREATE TEMPORARY TABLE Chest_Xray_TB AS  
  SELECT `person_id`, IF(`value_coded` = 1, 'Chest_Xray_TB', 'No') AS 'Chest_Xray_TB', `obs_datetime` FROM obs o
  LEFT JOIN `encounter` e ON o.`encounter_id`=e.`encounter_id` WHERE concept_id = 166143 AND e.`encounter_type` = 23 AND e.`form_id` = 53  AND o.`voided`= 0 AND `person_id` IN (SELECT * FROM Latest_IDs);
DROP TABLE IF EXISTS Culture_TB;
CREATE TEMPORARY TABLE Culture_TB AS  
  SELECT `person_id`, IF(`value_coded` = 1, 'Culture_TB', 'No') AS 'Culture_TB', `obs_datetime` FROM obs o
  LEFT JOIN `encounter` e ON o.`encounter_id`=e.`encounter_id` WHERE concept_id = 166144 AND e.`encounter_type` = 23 AND e.`form_id` = 53  AND o.`voided`= 0 AND `person_id` IN (SELECT * FROM Latest_IDs);
DROP TABLE IF EXISTS Sputum_AFB_Result;
CREATE TEMPORARY TABLE Sputum_AFB_Result AS 
  SELECT `person_id`, value_coded AS 'Sputum_AFB_Result', `obs_datetime` FROM obs o
  LEFT JOIN `encounter` e ON o.`encounter_id`=e.`encounter_id` WHERE concept_id = 165968 AND e.`encounter_type` = 23 AND e.`form_id` = 53  AND o.`voided`= 0 AND `person_id` IN (SELECT * FROM Latest_IDs);
DROP TABLE IF EXISTS GeneXpert_Result;
CREATE TEMPORARY TABLE GeneXpert_Result AS 
  SELECT `person_id`, value_coded AS 'GeneXpert_Result', `obs_datetime` FROM obs o
  LEFT JOIN `encounter` e ON o.`encounter_id`=e.`encounter_id` WHERE concept_id = 165975 AND e.`encounter_type` = 23 AND e.`form_id` = 53  AND o.`voided`= 0 AND `person_id` IN (SELECT * FROM Latest_IDs);
DROP TABLE IF EXISTS Chest_Xray_Result;
CREATE TEMPORARY TABLE Chest_Xray_Result AS
  SELECT `person_id`, value_coded AS 'Chest_Xray_Result', `obs_datetime` FROM obs o
  LEFT JOIN `encounter` e ON o.`encounter_id`=e.`encounter_id` WHERE concept_id = 165972 AND e.`encounter_type` = 23 AND e.`form_id` = 53  AND o.`voided`= 0 AND `person_id` IN (SELECT * FROM Latest_IDs);
DROP TABLE IF EXISTS Culture_Result;
CREATE TEMPORARY TABLE Culture_Result AS
  SELECT `person_id`, value_coded AS 'Culture_Result', `obs_datetime` FROM obs o
  LEFT JOIN `encounter` e ON o.`encounter_id`=e.`encounter_id` WHERE concept_id = 165969 AND e.`encounter_type` = 23 AND e.`form_id` = 53  AND o.`voided`= 0 AND `person_id` IN (SELECT * FROM Latest_IDs);
...
LEFT JOIN `patient_identifier` p2 ON o.`person_id` = p2.`patient_id` AND p2.`identifier_type` = 4,
(SELECT @row_no := 0) X, (SELECT @prev_val := '') Y 
WHERE o.`concept_id` IN (166141, 166142, 166143, 166144)  AND e.`encounter_type` = 23 AND e.`form_id` = 53  AND o.`voided` = 0 AND o.`obs_datetime` <= @Reporting_Date AND `person_id` IN (SELECT * FROM Latest_IDs)
ORDER BY o.person_id, o.`obs_datetime` DESC; 
DROP TABLE IF EXISTS Last_All_TB_Investig;
CREATE TEMPORARY TABLE Last_All_TB_Investig AS
SELECT a.Occurrence, a.person_id, a.Pepid, a.obs_datetime Date_TB_Investig,  f.Sputum_AFB_TB, c4.name Sputum_AFB_Result,
...
DROP TABLE IF EXISTS CIHP_EAC_Data;CREATE TEMPORARY TABLE CIHP_EAC_Data AS
SELECT @row_no := IF(@prev_val = b.person_id , @row_no + 1, 1) AS Occurrence, @prev_val := b.person_id AS person_id,
b.`obs_datetime`, b.`encounter_datetime`, b.`concept_id`, b.`encounter_id`, b.`value_coded`, b.EAC_Session_Type, b.EAC_ARV_Plan 
FROM (SELECT o.person_id, o.`obs_datetime`, e.`encounter_datetime`, o.`concept_id`,o.`encounter_id`, o.`value_coded`, IF(o.`concept_id`=166097, ConceptName(o.`value_coded`), NULL) EAC_Session_Type, a.EAC_ARV_Plan 
FROM `obs` o
LEFT JOIN `encounter` e ON o.`person_id`=e.`patient_id` AND o.`encounter_id`=e.`encounter_id` 
LEFT JOIN (SELECT DISTINCT `person_id`, `obs_datetime`, `encounter_id`, IF(`concept_id`=165771, ConceptName(`value_coded`), NULL) EAC_ARV_Plan FROM obs WHERE `concept_id`=165771  AND `person_id` IN (SELECT * FROM Latest_IDs)) AS a 
ON a.`person_id`=o.`person_id` AND a.`encounter_id`=e.`encounter_id`
WHERE o.`concept_id`=166097 AND e.`encounter_type`=32 AND e.`encounter_datetime`<= @Reporting_Date AND o.`person_id` IN (SELECT * FROM Latest_IDs) ORDER BY o.person_id, o.`obs_datetime` DESC) b,
(SELECT @row_no := 0, @prev_patient := NULL) vars
HAVING Occurrence = 1;
...
IF(o.`concept_id`=166097, ConceptName(o.`value_coded`), NULL) EAC_Session_Type, a.EAC_ARV_Plan FROM `obs` o
LEFT JOIN `encounter` e ON o.`person_id`=e.`patient_id` AND o.`encounter_id`=e.`encounter_id` 
LEFT JOIN (SELECT DISTINCT `person_id`, `obs_datetime`, `encounter_id`, IF(`concept_id`=165771, ConceptName(`value_coded`), NULL) EAC_ARV_Plan FROM obs WHERE `concept_id`=165771 
AND `person_id` IN (SELECT * FROM Latest_IDs)) AS a 
ON a.`person_id`=o.`person_id` AND a.`encounter_id`=e.`encounter_id`,
(SELECT @row_no := 0) X, (SELECT @prev_val := '') Y 
WHERE o.`concept_id`=166097 AND e.`encounter_type`=32 AND e.`encounter_datetime`<= @Reporting_Date AND o.`person_id` IN (SELECT * FROM Latest_IDs) ORDER BY o.person_id, o.`obs_datetime` ASC;
DROP TABLE IF EXISTS VL_Before_EAC;CREATE TEMPORARY TABLE VL_Before_EAC AS
SELECT a.person_id, MAX(a.DateOfCurrentVL)DateOfCurrentVL FROM CIHP_ViralLoad_Data a LEFT JOIN CIHP_EAC_Data2 b ON a.person_id=b.person_id
WHERE a.DateOfCurrentVL < b.encounter_datetime AND a.CurrentVL>50 AND b.Occurrence=1 GROUP BY a.person_id;
DROP TABLE IF EXISTS VL_After_EAC;CREATE TEMPORARY TABLE VL_After_EAC AS
...
DROP TABLE IF EXISTS `validation_linelist`.Quick_Linelist;

CREATE TABLE `validation_linelist`.Quick_Linelist AS
SELECT 'CIHP' IP, @State State, @SurgeCommand SurgeCommand, @LGA LGA, @FacilityName FacilityName, /*a.`patient_id`,*/CONCAT(a.Pepid, "-", @DATIMCode) Pepid_datim, a.`patient_id`,
a.Pepid, b.HospitalNo, IF((a.sex = 'F'), 'Female', 'Male') Sex, k.KP_Type, a.`encounter_datetime` Date_Enrolled,
TIMESTAMPDIFF(YEAR, a.DOB, c.ARTStartDate) AS AgeAtStartofART,
TIMESTAMPDIFF(MONTH, a.DOB, c.ARTStartDate) AS AgeAtStart_InMonth, TIMESTAMPDIFF(YEAR, a.DOB, @Reporting_Date) AS CurrentAge,
...
AND d.Date_Returned IS NOT NULL, d.Date_Returned, d.Date_of_Termination)Date_of_Termination, d.Facility_Transferred_To, f.Last_EAC_Session_Date,
  f.EAC_Session_Type, f.EAC_ARV_Plan, f.Date_VL_Before_EAC, f.VL_Before_EAC, f.Date_VL_After_EAC, f.VL_After_EAC, 
  CONCAT("QL", LEFT(UUID(), 2),RIGHT(YEAR(@Reporting_Date), 2),"-" , LEFT(UUID(), 5),LPAD(MONTH(@Reporting_Date), 2, '0'),"-", LEFT(UUID(), 3), LPAD(DAY(@Reporting_Date), 2, '0')) List_Parameters 
FROM `CIHP_Patient` a 
LEFT JOIN CIHP_Hop b ON a.`patient_id`=b.`patient_id`
LEFT JOIN CIHP_ART_List c ON a.`patient_id`=c.`patient_id`
LEFT JOIN Cihp_Client_Tracking d ON a.`patient_id`=d.`person_id`
...
FROM `obs` o LEFT JOIN `encounter` e ON o.`encounter_id`=e.`encounter_id`,
(SELECT @row_no := 0) X, (SELECT @prev_val := '') Y 
WHERE o.`concept_id` =1659  AND e.`encounter_type` = 12 AND e.`form_id` = 14  AND o.`voided` = 0 AND o.`obs_datetime` <= @Reporting_Date  AND `person_id` IN (SELECT * FROM Latest_IDs)
ORDER BY o.person_id, o.`obs_datetime` DESC)a WHERE a.Occurrence=1) g ON a.patient_id=g.person_id 
LEFT JOIN (SELECT a.person_id, a.Initial_CD4_Date,a.initial_CD4 FROM
(SELECT @row_no := IF(@prev_val = o.person_id , @row_no + 1, 1) AS Occurrence, @prev_val := o.person_id AS person_id, 
o.`obs_datetime` Initial_CD4_Date,o.`value_numeric` initial_CD4
FROM `obs` o LEFT JOIN `encounter` e ON o.`encounter_id`=e.`encounter_id`,
(SELECT @row_no := 0) X, (SELECT @prev_val := '') Y 
WHERE o.`concept_id` =5497  AND e.`encounter_type` = 11 AND e.`form_id` = 21  AND o.`voided` = 0 AND o.`obs_datetime` <= @Reporting_Date AND `person_id` IN (SELECT * FROM Latest_IDs)
ORDER BY o.person_id, o.`obs_datetime` ASC)a WHERE a.Occurrence=1) h ON a.patient_id=h.person_id
LEFT JOIN (SELECT a.person_id, a.Last_CD4_Date,a.Last_CD4 FROM
(SELECT @row_no := IF(@prev_val = o.person_id , @row_no + 1, 1) AS Occurrence, @prev_val := o.person_id AS person_id, 
o.`obs_datetime` Last_CD4_Date,o.`value_numeric` Last_CD4
...
(SELECT @row_no := IF(@prev_val = e.patient_id , @row_no + 1, 1) AS Occ, @prev_val := e.patient_id AS patient_id, e.`encounter_datetime`, e.`encounter_id`
FROM `encounter` e, (SELECT @row_no := 0) X, (SELECT @prev_val := '') Y
WHERE e.`encounter_type` = 12 AND e.`form_id` = 14 AND e.`voided` = 0 AND e.`encounter_datetime` <= @Reporting_Date AND `person_id` IN (SELECT * FROM Latest_IDs)
ORDER BY e.patient_id, e.`encounter_datetime` DESC) a 
LEFT JOIN (SELECT person_id, `value_numeric` Systolic_BP, encounter_id FROM obs WHERE `concept_id`=5085 AND `voided` = 0) b ON a.patient_id=b.person_id AND a.encounter_id=b.encounter_id
LEFT JOIN (SELECT person_id, `value_numeric` Diastolic_BP, encounter_id FROM obs WHERE `concept_id`=5086 AND `voided` = 0) c ON a.patient_id=c.person_id AND a.encounter_id=c.encounter_id
LEFT JOIN (SELECT person_id, `value_datetime` Next_Clinical_Appt_Date, encounter_id FROM obs WHERE `concept_id`=5096 AND `voided` = 0) d ON a.patient_id=c.person_id AND a.encounter_id=d.encounter_id
...
#SELECT * FROM `validation_linelist`.Quick_Linelist;

DROP TABLE IF EXISTS `validation_linelist`.Last_Encounter_ID;
CREATE TABLE `validation_linelist`.Last_Encounter_ID AS
SELECT MAX(`encounter_id`) AS Last_Enc_id, MAX(`date_created`) AS Last_Date_time FROM `openmrs`.`encounter`;

#Update and Insert for new clients
UPDATE `validation_linelist`.`full_linelist_for_quick_list_update` a
JOIN  `validation_linelist`.`quick_linelist` b ON a.Pepid_datim = b.Pepid_datim
SET 

a. Sex  = b. Sex ,
a. KP_Type  = b. KP_Type ,
a. Date_Enrolled  = b. Date_Enrolled ,
a. AgeAtStartofART  = b. AgeAtStartofART ,
a. AgeAtStart_InMonth  = b. AgeAtStart_InMonth ,
a. CurrentAge  = b. CurrentAge ,
a. ARTStartDate  = b. ARTStartDate ,
a. LastPickupDate  = b. LastPickupDate ,
a. NextAppmt  = b. NextAppmt ,
a. DaysOfARVRefill  = b. DaysOfARVRefill ,
a. RegLineAtStart  = b. RegLineAtStart ,
a. ARTRegAtStart  = b. ARTRegAtStart ,
a. CurrentRegLine  = b. CurrentRegLine ,
a. CurrentARTReg  = b. CurrentARTReg ,
a. PregStatus  = b. PregStatus ,
a. CurrentVL  = b. CurrentVL ,
a. DateOfCurrentVL  = b. DateOfCurrentVL ,
a. ViralLoadIndication  = b. ViralLoadIndication ,
a. Date_Result_Received  = b. Date_Result_Received ,
a. Last_VL_Sample_Date  = b. Last_VL_Sample_Date ,
a. Initial_CD4_Date  = b. Initial_CD4_Date ,
a. initial_CD4  = b. initial_CD4 ,
a. Last_CD4_Date  = b. Last_CD4_Date ,
a. Last_CD4  = b. Last_CD4 ,
a. CurrentARTStatus_28Days  = b. CurrentARTStatus_28Days ,
...
a. Address  = b. Address ,
a. Pill_Balance  = b. Pill_Balance ,
a. Next_Pickup_Date  = b. Next_Pickup_Date ,
a. MonthOnART  = b. MonthOnART ,
a. Last_Weight  = b. Last_Weight ,
a. Last_Weight_Date  = b. Last_Weight_Date ,
a. Biometrics_Captured  = b. Biometrics_Captured ,
a. Recapture_Count  = b. Recapture_Count ,
a. Date_Last_Recaptured  = b. Date_Last_Recaptured ,
a. Eligible_For_IPT  = b. Eligible_For_IPT ,
a. Date_IPT_start  = b. Date_IPT_start ,
a. Outcome_of_IPT  = b. Outcome_of_IPT ,
a. Date_of_Outcome  = b. Date_of_Outcome ,
a. Last_Clinic_Visit_Date  = b. Last_Clinic_Visit_Date ,
a. Systolic_BP  = b. Systolic_BP ,
a. Diastolic_BP  = b. Diastolic_BP ,
a. Next_Clinical_Appt_Date  = b. Next_Clinical_Appt_Date ,
a. Last_TB_Screening_Date  = b. Last_TB_Screening_Date ,
a. Last_TB_Screening_Status  = b. Last_TB_Screening_Status ,
a. TB_Investigations  = b. TB_Investigations ,
a. Investig_Result  = b. Investig_Result ,
a. Date_TB_Investig  = b. Date_TB_Investig ,
a. Reason_for_Tracking  = b. Reason_for_Tracking ,
a. Tracking_Date  = b. Tracking_Date ,
a. Reason_for_Defaulting1  = b. Reason_for_Defaulting1 ,
a. Reason_for_Termination  = b. Reason_for_Termination ,
a. Date_of_Termination  = b. Date_of_Termination ,
a. Facility_Transferred_To  = b. Facility_Transferred_To ,
a. Last_EAC_Session_Date  = b. Last_EAC_Session_Date ,
a. EAC_Session_Type  = b. EAC_Session_Type ,
a. EAC_ARV_Plan  = b. EAC_ARV_Plan ,
a. Date_VL_Before_EAC  = b. Date_VL_Before_EAC ,
a. VL_Before_EAC  = b. VL_Before_EAC ,
a. Date_VL_After_EAC  = b. Date_VL_After_EAC ,
a. VL_After_EAC  = b. VL_After_EAC ,
a. List_Parameters  = b. List_Parameters ;

INSERT INTO `validation_linelist`.`full_linelist_for_quick_list_update`
SELECT b.*
FROM `validation_linelist`.`quick_linelist` b
LEFT JOIN `validation_linelist`.`full_linelist_for_quick_list_update` a
    ON a.Pepid_datim = b.Pepid_datim
WHERE a.Pepid_datim IS NULL;

UPDATE `validation_linelist`.`full_linelist_for_quick_list_update`
SET `List_Parameters` = 
CONCAT("QL", LEFT(UUID(), 2),RIGHT(YEAR(@Reporting_Date), 2),"-" , LEFT(UUID(), 5),LPAD(MONTH(@Reporting_Date), 2, '0'),"-", LEFT(UUID(), 3), LPAD(DAY(@Reporting_Date), 2, '0'));
...
cur.execute("""DROP PROCEDURE IF EXISTS `eac_cascade_linelist`""")
cur.execute("""
CREATE PROCEDURE `eac_cascade_linelist`()
  BEGIN


SET @FacilityName := (SELECT `property_value` FROM `global_property` WHERE `property` = 'Facility_Name');
SET @DATIMCode := (SELECT `property_value` FROM `global_property` WHERE `property` = 'facility_datim_code');
SET @SurgeCommand := (SELECT SurgeCommand FROM CIHP_ListOfFacility WHERE Datim_Code = @DATIMCode);
SET @LGA := (SELECT LGA FROM CIHP_ListOfFacility WHERE Datim_Code = @DATIMCode);
SET @State := (SELECT State FROM CIHP_ListOfFacility WHERE Datim_Code = @DATIMCode);


DROP TABLE IF EXISTS `validation_linelist`.eac_linelist_for_ex;
CREATE TABLE `validation_linelist`.eac_linelist_for_ex AS

SELECT 
  'CIHP' AS IP,
  @State AS State,
  @SurgeCommand AS SurgeCommand,
  @LGA AS LGA,
  @FacilityName AS FacilityName,
  CONCAT(p2.`identifier`, "-", @DATIMCode) AS Pepid_datim,
  p2.`identifier` AS Pepid,
  e.`patient_id`,
  art.ARTStartDate,
  TIMESTAMPDIFF(YEAR, per.birthdate, CURDATE()) AS Current_Age,
  cr.Indication_For_VL,
  vl.VL_Result_Before_EAC,
  vl.Date_of_VL_Result_Before_EAC,
  eac_sessions.First_EAC_Session,
  eac_sessions.Date_of_First_EAC_Session,
  adherence.ARV_Adherence_First_Session,
  eac_sessions.Second_EAC_Session,
  eac_sessions.Date_of_Second_EAC_Session,
  adherence.ARV_Adherence_Second_Session,
  eac_sessions.Third_EAC_Session,
  eac_sessions.Date_of_Third_EAC_Session,
  adherence.ARV_Adherence_Third_Session,
  eac_sessions.Additional_EAC_Session,
  eac_sessions.Date_of_Additional_EAC_Session,
  adherence.ARV_Adherence_Additional_Session,
  vl_after_eac.Sample_Collection_Date_After_EAC,
  vl_after_eac.VL_Result_After_EAC,
  vl_after_eac.Date_of_VL_Result_After_EAC,
  CONCAT(LEFT(UUID(), 4),RIGHT(YEAR(@Reporting_Date), 2),"-" , LEFT(UUID(), 5),
LPAD(MONTH(@Reporting_Date), 2, '0'),"-", LEFT(UUID(), 3), LPAD(DAY(@Reporting_Date), 2, '0')) List_Parameters
FROM `encounter` e

INNER JOIN (
  SELECT patient_id, MAX(encounter_datetime) AS latest_eac_date FROM encounter
  WHERE encounter_type = 32 AND form_id = 69 AND voided = 0
  GROUP BY patient_id) latest_eac ON e.patient_id = latest_eac.patient_id AND e.encounter_datetime = latest_eac.latest_eac_date

LEFT JOIN (SELECT o.person_id,
  MIN(o.value_datetime) AS ARTStartDate FROM `obs` o
  INNER JOIN `encounter` en ON o.encounter_id = en.encounter_id WHERE o.concept_id = 159599
    AND o.voided = 0
    AND o.value_datetime IS NOT NULL
    AND en.form_id = 56
    AND en.voided = 0
  GROUP BY o.person_id) art ON e.`patient_id` = art.`person_id`

LEFT JOIN (
  SELECT 
    o.person_id, o.obs_datetime,
    CASE o.value_coded
      WHEN 162080 THEN 'Baseline (6 months after ART initiation)'
      WHEN 161236 THEN 'Routine (every 12 months)'
      WHEN 162082 THEN 'Confirmation (3-6 months after intense adherence counselling)'
      WHEN 162081 THEN 'Repeat test'
      WHEN 163523 THEN 'Clinical failure'
      WHEN 160566 THEN 'Immunological failure'
      WHEN 165978 THEN 'PMTCT 32-36 weeks gestation'
      ELSE 'Not Indicated'
    END AS Indication_For_VL
  FROM obs o
  INNER JOIN (
    SELECT person_id, MAX(obs_datetime) AS max_obs_date FROM obs WHERE concept_id = 164980
      AND value_coded IN (162080, 161236, 162082, 162081, 163523, 160566, 165978)
      AND voided = 0
      AND encounter_id IN (SELECT encounter_id FROM encounter WHERE form_id = 21)
    GROUP BY person_id
  ) latest ON o.person_id = latest.person_id AND o.obs_datetime = latest.max_obs_date
  WHERE o.concept_id = 164980 AND o.voided = 0
) cr ON e.patient_id = cr.person_id


LEFT JOIN (
  SELECT 
    o.person_id,
    o.obs_datetime AS Date_of_VL_Result_Before_EAC,
    o.value_numeric AS VL_Result_Before_EAC
  FROM obs o
  INNER JOIN (
    SELECT o1.person_id, MAX(o1.obs_datetime) AS latest_vl_before_eac
    FROM obs o1
    INNER JOIN obs eac ON o1.person_id = eac.person_id
    WHERE o1.concept_id = 856
      AND o1.voided = 0
      AND o1.value_numeric IS NOT NULL
      AND eac.concept_id = 166097
      AND eac.value_coded = 165643
      AND eac.voided = 0
      AND o1.obs_datetime < eac.obs_datetime
    GROUP BY o1.person_id
  ) latest ON o.person_id = latest.person_id AND o.obs_datetime = latest.latest_vl_before_eac
  WHERE o.concept_id = 856 AND o.voided = 0 AND o.value_numeric IS NOT NULL
) vl ON e.patient_id = vl.person_id

LEFT JOIN (
  SELECT 
    person_id,
    MAX(CASE WHEN value_coded = 165643 THEN '1st EAC Session' END) AS First_EAC_Session,
    MAX(CASE WHEN value_coded = 165643 THEN obs_datetime END) AS Date_of_First_EAC_Session,
    
    MAX(CASE WHEN value_coded = 165644 THEN '2nd EAC Session' END) AS Second_EAC_Session,
    MAX(CASE WHEN value_coded = 165644 THEN obs_datetime END) AS Date_of_Second_EAC_Session,
    
    MAX(CASE WHEN value_coded = 165645 THEN '3rd EAC Session' END) AS Third_EAC_Session,
    MAX(CASE WHEN value_coded = 165645 THEN obs_datetime END) AS Date_of_Third_EAC_Session,
    
    MAX(CASE WHEN value_coded = 5622 THEN 'Other EAC Session' END) AS Additional_EAC_Session,
    MAX(CASE WHEN value_coded = 5622 THEN obs_datetime END) AS Date_of_Additional_EAC_Session

  FROM obs
  WHERE concept_id = 166097
    AND value_coded IN (165643, 165644, 165645, 5622)
    AND voided = 0
  GROUP BY person_id
) eac_sessions ON e.patient_id = eac_sessions.person_id

LEFT JOIN (
  SELECT 
    person_id,
    MAX(CASE WHEN session_type = '1st EAC Session' THEN adherence END) AS ARV_Adherence_First_Session,
    MAX(CASE WHEN session_type = '2nd EAC Session' THEN adherence END) AS ARV_Adherence_Second_Session,
    MAX(CASE WHEN session_type = '3rd EAC Session' THEN adherence END) AS ARV_Adherence_Third_Session,
    MAX(CASE WHEN session_type = 'Additional EAC Session' THEN adherence END) AS ARV_Adherence_Additional_Session
  FROM (
    SELECT 
      o.person_id,
      o.obs_datetime,
      CASE eac.value_coded
        WHEN 165643 THEN '1st EAC Session'
        WHEN 165644 THEN '2nd EAC Session'
        WHEN 165645 THEN '3rd EAC Session'
        WHEN 5622 THEN 'Additional EAC Session'
      END AS session_type,
      CASE o.value_coded
        WHEN 165287 THEN 'Good'
        WHEN 165288 THEN 'Fair'
        WHEN 165289 THEN 'Poor'
        ELSE NULL
      END AS adherence
    FROM obs o
    INNER JOIN obs eac ON o.person_id = eac.person_id 
        AND o.encounter_id = eac.encounter_id 
        AND eac.concept_id = 166097 
        AND eac.value_coded IN (165643, 165644, 165645, 5622)
    WHERE o.concept_id = 165290
      AND o.value_coded IN (165287, 165288, 165289)
      AND o.voided = 0
      AND eac.voided = 0
  ) session_adherence
  GROUP BY person_id
) adherence ON e.patient_id = adherence.person_id

LEFT JOIN (
  SELECT 
    person_id,
    MIN(CASE WHEN concept_id = 159951 THEN obs_datetime END) AS Sample_Collection_Date_After_EAC,
    MIN(CASE WHEN concept_id = 856 THEN value_numeric END) AS VL_Result_After_EAC,
    MIN(CASE WHEN concept_id = 856 THEN obs_datetime END) AS Date_of_VL_Result_After_EAC
  FROM obs
  WHERE voided = 0
    AND (
      concept_id = 159951 OR  -- Sample collection date
      (concept_id = 856 AND value_numeric IS NOT NULL)  -- VL result
    )
    AND obs_datetime > (
      SELECT MAX(obs_datetime)
      FROM obs
      WHERE person_id = obs.person_id
        AND concept_id = 166097
        AND value_coded = 5622  -- Other EAC session
        AND voided = 0
    )
  GROUP BY person_id
) vl_after_eac ON e.patient_id = vl_after_eac.person_id

LEFT JOIN `patient_identifier` p2 ON e.`patient_id` = p2.`patient_id` 
  AND p2.`identifier_type` = 4 
  AND p2.`voided` = 0
  
LEFT JOIN `person` per ON e.`patient_id` = per.`person_id`;

SELECT * FROM `validation_linelist`.eac_linelist_for_ex;
    END
""")


cur.execute("""DROP PROCEDURE IF EXISTS `CIHP_Extended_Treatment_Linelist`""")
cur.execute("""
  CREATE PROCEDURE `CIHP_Extended_Treatment_Linelist`()
  BEGIN
  #Populate Extended Line list
TRUNCATE `validation_linelist`.`Extended_Treatment_Linelist`;
INSERT INTO `validation_linelist`.`Extended_Treatment_Linelist`
(IP,
State,
SurgeCommand,
LGA,
...
CurrentARTReg,
Switch_Substitution,
Date_AtStart_of_Current_ARTregimen,
CurrentVL,
DateOfCurrentVL,
ViralLoadIndication,
Date_Result_Received,
Last_VL_Sample_Collection_Date,
Indication_for_CD4,
CD4_LFA_Result,
Date_CD4_LFA,
CD4_Percentage,
Date_of_CD4_Percentage,
CD4_cell_count,
Date_of_CD4_Cell_count,
TB_LF_LAM_Result,
Date_TB_LF_LAM_Result_Received,
Serology_For_CrAg_Result,
Date_Serology_For_CrAg_Result_Received,
CSF_For_CrAg_Result,
Date_CSF_For_CrAg_Result_Received,
Cryptococcal_Screening_Status,
ICE_WHO_Staging,
RTT,
RTT_Date,
RTT_after_IIT_Lessthan_12months,
RTT_Outcome,
...
Reason_for_tracking,
Tracking_Outcome,
Date_of_Tracking_Outcome,
Reason_for_Discontinuation,
Date_of_Discontinuation,
Facility_Transferred_To,
VL_result__before_EAC,
Date_of_VL_before_EAC_was_received,
EAC_start_Date,
EAC_Adherence_Outcome,
Date_of_2nd_EAC,
2nd_EAC_Adherence_Outcome,
Date_of_3rd_EAC,
3rd_EAC_Adherence_Outcome,
Date_of_Additional_EAC,
Date_of_Post_EAC_VL_sample_collection,
Post_EAC_VL_result,
Date_of_Post_EAC_VL_result_Received,
DSD_Status,
Date_of_devolvement_to_DSD_model,
DSD_Model,
DSD_Model_Type,
Date_Enrolled_into_OTZ,
OTZ_Disclosure_status,
OTZ_Date_of_Full_Dsiclosure,
Date_of_commencement_of_OTZ_Modules,
Number_of_OTZ_modules_completed,
Date_of_Completion_of_Final_Module,
OTZ_Program_Outcome,
Date_transitioned_into_Adult_care,
Biometrics_Captured,
Recapture_Count,
Date_Last_Recaptured,
List_Parameters
)SELECT 
a.IP,
a.State,
...
a.HospitalNo,
a.Sex,
CASE WHEN a.PregStatus IS NOT NULL THEN a.LastPickupDate ELSE NULL END,
a.PregStatus,
a.Date_Enrolled,
a.DOB,
a.AgeAtStartofART,
...
a.NextAppmt,
a.DaysOfARVRefill,
a.CurrentARTStatus_28Days,
a.CurrentRegLine,
a.CurrentARTReg,
e.`Switch/Substitution`,
"",
a.CurrentVL,
a.DateOfCurrentVL,
a.ViralLoadIndication,
a.Date_Result_Received,
a.Last_VL_Sample_Date,
c.Indication_for_CD4,
c.CD4_LFA_Result,
c.Date_of_CD4_LFA_Result,
c.CD4_Percent,
c.CD4_Percent_Date,
c.CD4_Count,
c.CD4_Count_Date,
c.TB_LF_LAM_Result,
c.Date_TB_LF_LAM_Result_Received,
c.Serology_For_CrAg_Result,
c.Date_Serology_For_CrAg_Result_Received,
c.CSF_For_CrAg_Result,
c.Date_CSF_For_CrAg_Result_Received,
c.Cryptococcal_Screening_Status,
c.ICE_WHO_Staging,
"",
"",
"",
...
a.Last_Weight_Date,
a.Eligible_For_IPT,
a.Last_TB_Screening_Date,
a.Last_TB_Screening_Status,
a.TB_Investigations,
a.Investig_Result,
a.Date_TB_Investig,
a.Date_IPT_start,
a.Outcome_of_IPT,
a.Date_of_Outcome,
a.Last_Clinic_Visit_Date,
a.Systolic_BP,
a.Diastolic_BP,
d.Tracking_Date,
d.Reason_For_Tracking,
CASE
    -- All three reasons are present
    WHEN d.Reason_for_Discontinuation IS NOT NULL
         AND d.Returned_to_Treatment_Status IS NOT NULL
         AND d.Reason_LTFU IS NOT NULL THEN
        CASE
            WHEN GREATEST(d.Date_of_Discontinuation, d.Date_Returned, Date_LTFU) = d.Date_of_Discontinuation THEN d.Reason_for_Discontinuation
            WHEN GREATEST(d.Date_of_Discontinuation, d.Date_Returned, Date_LTFU) = d.Date_Returned THEN d.Returned_to_Treatment_Status
            WHEN GREATEST(d.Date_of_Discontinuation, d.Date_Returned, Date_LTFU) = d.Date_LTFU THEN d.Reason_LTFU
        END

    -- Two reasons: Discontinuation + Returned
    WHEN d.Reason_for_Discontinuation IS NOT NULL
         AND d.Returned_to_Treatment_Status IS NOT NULL THEN
        CASE
            WHEN d.Date_of_Discontinuation >= d.Date_Returned THEN d.Reason_for_Discontinuation
            ELSE d.Returned_to_Treatment_Status
        END
    ...
    WHEN d.Reason_for_Discontinuation IS NOT NULL
         AND d.Reason_LTFU IS NOT NULL THEN
        CASE
            WHEN d.Date_of_Discontinuation >= d.Date_LTFU THEN d.Reason_for_Discontinuation
            ELSE d.Reason_LTFU
        END

    -- Returned + LTFU
    WHEN d.Returned_to_Treatment_Status IS NOT NULL
         AND d.Reason_LTFU IS NOT NULL THEN
        CASE
            WHEN d.Date_Returned >= d.Date_LTFU THEN d.Returned_to_Treatment_Status
            ELSE d.Reason_LTFU
        END

    -- Only one present
    WHEN d.Reason_for_Discontinuation IS NOT NULL THEN d.Reason_for_Discontinuation
    WHEN d.Returned_to_Treatment_Status IS NOT NULL THEN d.Returned_to_Treatment_Status
    WHEN d.Reason_LTFU IS NOT NULL THEN d.Reason_LTFU

    ELSE NULL
END,
"",
d.Reason_for_Discontinuation,
a.Date_of_Termination,
a.Facility_Transferred_To,
a.VL_Before_EAC,
a.Date_VL_Before_EAC,
"",
"",
"",
"",
...
e.DSD_Model,
"",
b.Date_Enrolled_Into_OTZ,
b.OTZ_Disclosure_Status,
b.OTZ_Date_of_Full_Disclosure,
b.Date_of_Commencement_of_OTZ_Module,
b.Number_of_OTZ_Modules_Completed,
b.Date_of_Final_Module_Completion,
b.OTZ_Program_Outcome,
b.Date_Transitioned_to_Adult_Clinic,
a.Biometrics_Captured,
a.Recapture_Count,
a.Date_Last_Recaptured,
a.List_Parameters

FROM `validation_linelist`.`full_linelist_for_quick_list_update` a
LEFT JOIN `validation_linelist`.`otz_linelist_for_ex` b ON a.`Pepid_datim` = b.`Pepid_datim`
LEFT JOIN `validation_linelist`.`ahd_linelist_for_ex` c ON a.`Pepid_datim` = c.`Pepid_datim`
LEFT JOIN `validation_linelist`.`ctd_linelist_for_ex` d ON a.`Pepid_datim` = d.`Pepid_datim`
LEFT JOIN `validation_linelist`.`full_pharmacy_complement_for_ex` e ON a.`Pepid_datim` = e.`Pepid_datim`;
